...
Two capabilities are available today — **Create Team** (a new team from scratch) and **Validate Team** (end-to-end validation of a team Create Team generated in your clone, read from that run's context file — two of its checks trust the results that run recorded there, so it is a check on a generated team, not on a hand-built one). Both are derived from the two rows in `_bmad/bme/_team-factory/module-help.csv`, which is what the module declares it offers.
