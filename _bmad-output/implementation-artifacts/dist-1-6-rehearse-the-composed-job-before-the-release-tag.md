...
| Date | Change |
|---|---|
| 2026-08-23 | **Round 1 on the retirement decision — closed; story moves to `done`.** Two layers: an adversarial audit of the tag-freeze retirement and an edge walk of the never-executed stable path. **Verdict on the retirement: SOUND-WITH-CONDITIONS** — the rule's own condition (*"until T41 clears"*) was literally satisfied and no argument for reinstatement survived, but it was executed as a deletion rather than a succession. **No defect was found in this story's implementation**; all findings concern the stable tag that follows it, and were filed as **T46/T47/T48/T49** rather than reopened here. Three record corrections landed with them: `ci.yml` claimed this story's rehearsal would confirm GNU `sort -V` — it did not, because publishing a prerelease sent the guard down its skip branch, exactly the FR19 gap this story's creation note had already flagged; the epic's ship gate cited the retired NFR1; and ADR-003 assigned the break-glass procedure to Story 1.6 when the epic places it in 1.7. **Left to the operator:** whether to write a successor rule gating the *stable* tag, since the retirement was an operator decision |
