...
**Acceptance Criteria:**
- **No line inside the block is dropped**, and any line the bound stranded is reported. `rows` and `unrecognized` partition the in-block lines that are neither blank nor comments; `outsideBlock` collects story-key-shaped lines the bound did not cover. *This is not an independently-derived cross-check* — the residue shares `inBlock` with the parse, so it detects a bound that closed too early and nothing else. An earlier draft of this AC claimed a different-predicate cross-check; no such computation was ever written, and stating it here made the mechanism sound stronger than it is.
...
*The finding that mattered most was about my own text, again.* Four code comments and one AC asserted properties the code does not have — the residue predicate described as *"blind to the block bound"* and *"counts every line anywhere in the file"* when it runs only inside `if (!inBlock)`; `balanced` described as catching a truncated parse when it is algebraically forced true for every input; and an AC claiming a different-predicate cross-check that was never written. Corrected to what is actually true, including what the mechanism does **not** guarantee. (A later delta review found this very sentence had said *"all four"* when there were five — the fifth survived in a test-file comment five lines above an edit made in the same hunk.) This is the third round in which the defect was a confident sentence rather than a line of code.
...
**2026-08-31 — delta review of the post-Round-3 remediation (not a fourth round).**

*Why it ran.* `code-review-convergence` caps review at three rounds, but also says applying a finding is not itself a reviewed change and that an earlier round's verdict must never silently extend to text it never saw. 172 lines were written after the Round 3 reviewers received their diff — seven regression tests, the corrected comments, the spec corrections — and commit `35b6b1bd`'s Description recorded the rounds without disclosing that the remediation was unreviewed. So the delta was reviewed; the rest of the change was not re-reviewed.

*Two HIGH, both the same defect as every previous round: a confident sentence outrunning the code.*
- The `superseded` label fix was applied to the console line and not to the CI annotation, which kept calling a correct dedup *"not scanned"* — on the noisier channel, the one read in the PR UI. The test could not see it, having omitted `githubActions`. There is now one `bucketLabel()` for both channels, asserted on both.
- The `&& balanced` guard — one of the two Priority-1 Round 3 fixes — shipped with **zero coverage**: removing it survived the whole suite. The test named for it died on an unrelated arithmetic assertion, and its comment claimed that kill proved the guard. No input can produce an imbalance, so the reporter is now exported and driven directly with fabricated figures, with the balanced case asserted quiet so the positive case means something.
