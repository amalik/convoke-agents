...
`phase` = `sensitize`. `outputs` derived from the table above: `hc5-signal-report-*.md, hc5-behavior-report-*.md, hc5-portfolio-report-*.md`. **Check the `output-location` column against Noah's real paths** — his artifacts are under `vortex-artifacts/`, which matches the existing column value used by Emma/Wade/Mila, but verify rather than assume.
