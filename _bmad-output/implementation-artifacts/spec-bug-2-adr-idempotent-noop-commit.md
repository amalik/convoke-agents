...
- [x] [Review][Defer] Broad `git add _bmad-output/planning-artifacts/` stages any modified sibling files [scripts/migrate-artifacts.js:399] — deferred, pre-existing. Spec I/O Matrix explicitly lists this as "Not in scope." Fix would narrow the add to `adrPath` + supersede target path.
