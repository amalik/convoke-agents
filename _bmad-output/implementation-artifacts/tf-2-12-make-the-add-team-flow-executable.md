...
**~~Probed and found sound~~ — THIS CLAIM WAS FALSE. Corrected at R2.** R1 asserted that a degenerate `configPath` does not make check 2 vacuous, citing a probe that returned `valid: false`. **The probe tested the composite verdict, not the check.** `valid` was false because check 4 failed on an unrelated ground; check 2 was passing vacuously the whole time — `''`, `' '`, `'a'` and bare `'config.yaml'` all satisfied it, the last meaning *any* module's config reference sufficed. Two independent reviewers found this in minutes. Fixed at R2 by rejecting an unusable `configPath` before comparing. **The transferable lesson: asserting on an aggregate verdict cannot tell you which component produced it.**

### Round 2 Review — 2026-09-10 (three independent adversarial layers)

Run via `/bmad-code-review` with **independent subagents**, deliberately: R1 was me reviewing my own work, and the layers found things that structure could not surface. 25 findings; **8 fixed here, 4 record corrections, 7 filed as pre-existing.** `npm test` 2259 / 2258 pass / 0 fail after (up 3 more on new guard tests).

**The layers converged on rejecting my R1 refinement, and they were right.** R1 kept an explicit `module=` as "an additional signal, not a required convention." The Acceptance Auditor showed that was not what the code did — when the attribute was present it was the **sole** authority and the ruled config-derivation was discarded, making normative the very convention option (b) was rejected for inventing. Both hunters independently showed the cost: the regex is unanchored over the whole activation **body**, so `core_module="…"`, `submodule="…"` and BMB's own `module = "stand-alone"` vocabulary each hijack it and hard-fail a correct agent — and BMB is the delegate that authors these files at §3a. **Removed. Decision 1(c) now implemented as ruled**, with the one existing test whose fixture encoded the invented convention rewritten to express wrongness through the config reference.

**Measured before deciding, at the operator's request.** Pure (c) corrects 3 of 6 behavioural cases and breaks none; it costs exactly one test. The uncomfortable part, measured and now documented in the validator itself: **under pure (c) check 4 has zero independent detection power** — 0 cases where check 2 passes and check 4 fails, because check 2's reference contains check 4's. It is kept as a separately-named check for its distinct diagnostic, not for coverage. Giving it real independent work — asserting the config reference sits inside a load/read instruction rather than merely appearing in prose — is **filed, not done**: today an agent that name-drops its config and never loads it returns `valid: true`.

**Two vacuous passes closed, both of them the T121 shape inside the anti-T121 repair.** An empty `agentFiles` array returned `valid: true` because `[].every()` is true — and both call sites key solely on `valid`, so a driver that failed to accumulate the array got a green gate with zero agents inspected. A degenerate `configPath` made check 2 pass against any agent. Both now fail loudly with caller-side error messages distinguishable from agent-side ones.

**Three documentation defects I introduced at R1, all found by the layers:** `contract_files` was absent from my `{generation_context}` definition while `end-to-end-validator.js:217` and `manifest-tracker.js:37` both iterate it — for a Sequential team `checkContractFiles` emits **zero** checks and passes vacuously, and the abort manifest omits every contract file. `activation_validation_results` was named without its required shape, and §5c never said to capture it. `{project-root}` was missing from a table asserting it defined every placeholder.

**The ordering hole the gate move opened, which my "accepted cost" note did not cover.** §3d still wrote `progress.generate.{agent_id}: "complete"` — but nothing validated the agents any more, so `spec-differ.js::findResumePoint` saw the phase finished and resumed an aborted run straight into step-05 carrying an unvalidated agent, with no route back to §3. And because §5a/§5b write before §5c runs, a plain re-run hits `config-creator.js:26`'s additive-only refusal and fails its own `expect`. §3d now writes `"generated"`, §5c promotes to `"complete"` on pass, and the abort path is documented.

**`npm run lint` says nothing about this story's code.** The script is `eslint --max-warnings 0 scripts/ index.js tests/` and `_bmad/` is explicitly ignored — R1's "lint clean" was true and carried no information about the one library file changed. Verified separately with `--no-ignore`: exit 0.

**Filed, not fixed (pre-existing, out of this story's scope):** JSON substituted into `node -e "…"` terminates the double-quoted shell string, so the `run:` blocks still cannot be pasted verbatim with real data — framework-wide and larger than this story; only the first `<activation>` block in a file is validated; quadratic scan on repeated unclosed `<activation` opens; null/undefined `moduleConfig` throws instead of reporting; `modulePath` is not separator-normalised while content is, so check 4 cannot pass on win32 from the CLI entry point; `normaliseConfigRef` anchors to the first `_bmad/` rather than the module's; and check 4's missing independent detection power described above.

**Verified true by the auditor, recorded so it is not re-litigated:** all six AC verdicts MET, Decision 2 followed faithfully with coherent §3/§5 numbering, **zero scope creep**, every "leave alone" instruction honoured, `buildManifest` synchronous, `writeRegistryBlock` byte-unchanged, and the test counts accurate.
...
- `tests/team-factory/activation-validator.test.js` (modified) — 3 real-agent tests added at R1; at R2 the wrong-module test rewritten and 3 guard tests added (15 total in file)
...
| 2026-09-10 | **R2 — three independent adversarial layers, 25 findings: 8 fixed, 4 record corrections, 7 filed.** All three converged on rejecting R1's `module=` refinement, which gave the invented convention sole authority and was hijacked by `core_module=` and BMB's own `module = "stand-alone"`; Decision 1(c) is now implemented as ruled. Two vacuous passes closed (empty `agentFiles`, degenerate `configPath`) — both the T121 shape inside the anti-T121 repair. Three placeholder-definition defects fixed, including `contract_files`, whose absence silently voided every Sequential contract check. §3d no longer marks agents complete before anything validates them. Census corrected a second time and **stated with its scope**: 12 tags in 11 agent `.md` files, all MANDATORY, none bare, none with `module=`. `npm test` 2259/2258/0 fail. |
