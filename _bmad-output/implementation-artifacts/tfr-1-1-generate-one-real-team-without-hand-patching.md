...
- [x] **[Review][Decision] AC#1 names the wrong invariant.** The real rule is *no block may interpolate a contributor-authored value into the payload; only path-valued placeholders from a documented list*. Rewriting it puts the gate's detection rule and `step-01` §4 both in scope; holding AC#1 as written makes the free-text class a new backlog row and leaves the blocks broken for anyone whose role sentence contains an apostrophe. *Disposition: resolved — AC#1 rewritten to the invariant (no block interpolates a contributor-authored value). The additional review found two blocks still breaking it at `review`; both fixed 2026-09-15.*
- [x] **[Review][Decision] Who owns the abort-path shell injection — `tfr-1-1` or `T165`?** `manifest-tracker.js:156` emits `rm "${entry.path}"`, unquoted. `output_directory: '_bmad-output/x"; rm -rf ~; echo "'` is ACCEPTED at all three call sites, reaches `config.yaml`, and the abort path hands the operator three commands. `T165`(a) already owns that function emitting a broken `rm` and the epic puts it out of scope; the predicate accepting shell metacharacters is mine and is the cheap fix. *Disposition: resolved in this story — the shared predicate rejects the characters live inside the abort path's double quotes; `isContainedOutputDirectory('_bmad-output/x"; rm -rf ~; echo "')` → `false`.*

#### [Review][Patch]

- [x] **[Review][Patch] Nothing creates `{context_path}`; §5a throws after writing `config.yaml`** [step-04-generate.md §1] — the table and `initContext`'s JSDoc both say "created by §1"; §1 has no `run:` block. Re-run is then refused as `already exists`. Found by all three layers. *Disposition: fixed — step-04 §1 has the `initContext` block.*
- [x] **[Review][Patch] 9 of 14 documented context keys have no producing block** [step-04-generate.md §Placeholders] — only `config_yaml_path`, `output_directory_path`, `module_help_csv_path`, `activation_validation_results`, `registry_wiring_result` are written. `agent_files` is read by §5c and §5d: §5c fails loudly, **§5d succeeds silently with empty personas — `T131` reintroduced as the default outcome.** `WORKFLOW-DIR-EXISTS` and `CONTRACT-FILE-EXISTS` emit zero checks; `buildManifest` returns 3 entries for a run that created dozens, so the abort path leaves the rest on disk. *Disposition: fixed — the context-keys table names an owner for every key and §1 gives the recorder shape; that recorder's own transport residue is `T170`.*
...
- [x] **[Review][Patch] `spec-parser` was silently relaxed and is absent from the agreement sweep** [output-directory.js::stripProjectRoot] — moving prefix-stripping into the base predicate propagated it to `parseSpec`, which now accepts the CONFIG shape in a SPEC field. That undoes a `tf-2-13` R2 ruling recorded in the module's own fixture comment. AC#3 names `spec-parser` first among three callers; the suite sweeps the predicate, `assertContainedOutputDirectory`, `buildConfigData` and `ensureOutputDirectory` instead. *Disposition: fixed — `spec-parser` calls `isRepoRelativeOutputDirectory`, which rejects the `{project-root}/` CONFIG shape, and its suite iterates the shared table.*
- [x] **[Review][Patch] No mutant kills the `..`-segment clause** [output-directory-cases.js] — deleting `output-directory.js:95`, the line its own docstring calls *"the one that took three attempts"*, changes **0 of 23** table cases and leaves 118/118 green. It is reachable only on backslash input, for which the table has no case. Direct AC#7 violation. *Disposition: resolved by deleting the clause — unreachable; see §Safety Analysis.*
- [x] **[Review][Patch] Containment is lexical only; a symlink under `_bmad-output/` escapes the repo** [output-directory.js] — the header asserts "resolve + normalise + contains-check are all three required"; there is no `realpathSync`. Either add resolution or correct the header. *Disposition: filed as `T169`; the header claim was corrected.*
- [x] **[Review][Patch] Null byte reproduces the two-site divergence the unification deleted** [output-directory.js] — `assertContainedOutputDirectory` accepts `_bmad-output/x\0y`, `buildConfigData` writes it, `ensureOutputDirectory` rejects it. Narrower than before, same shape: "closed" meant "narrowed". *Disposition: fixed — `isContainedOutputDirectory('_bmad-output/x\0y')` → `false`.*
- [x] **[Review][Patch] Backslash containment claim is false** [output-directory.js:88] — the check normalises `\`→`/`, resolution uses the raw value, so `_bmad-output\foo` is reported contained and created as a SIBLING of `_bmad-output` at the repo root. *Disposition: fixed — `isContainedOutputDirectory('_bmad-output\\foo')` → `false`.*
- [x] **[Review][Patch] `recordContext` loses agents under the documented usage** [run-context.js:147] — step-01 §4 says "one command per agent"; three concurrently leaves 1 of 3, exit 0, no warning. Read-modify-write with a per-PID temp name, so nothing can detect the loss. *Disposition: the per-agent usage was removed (step-01 §4 writes the file directly); the unlocked read-modify-write is filed as `T167`(d).*
- [x] **[Review][Patch] Shipped step files assert Task 6 work not in this commit** [step-05-validate.md:57, step-04-generate.md:23] — "which tfr-1-1 rewrote to ask a differential question" and the `vortex_baseline` row. `git diff` over `lib/validators/` is empty; `grep -c vortex_baseline` is 0. *Disposition: moot — Task 6 landed in `0db3aac8`.*
- [x] **[Review][Patch] AC#1 states 12 blocks; the tree has 13** [story] — and the commit message uses 11, the gate prints 13. `derive-counts-from-source`. *Disposition: fixed — AC#1 states no count and gives the command.*
- [x] **[Review][Patch] `{context_path}` and `{scope_path}` are never given a concrete location** [step-01, step-04] — the only path placeholders with no filename. The story's own Safety Analysis requires the context file to sit outside `_bmad-output/`; nothing names or enforces that. *Disposition: fixed — both Placeholders tables name the file.*
- [x] **[Review][Patch] `{spec_path}` is documented repo-relative while every sibling is `{project-root}`-absolute, and is undefined in step-02 and step-05** [3 step files] — resolves against the executor's cwd; the "defined nowhere" class the story quotes twice. *Disposition: filed as `T168`.*
- [x] **[Review][Patch] §5a-ii's confirm command uses `require()`, imposing an undocumented `.json` extension** [step-04-generate.md:113] — `SyntaxError` on a correct run if the path lacks it. *Disposition: fixed — it uses `readContext`.*
...
- [x] **[Review][Patch] A capability containing `|` is silently split** [step-01-scope.md:98] — pipes were chosen so a comma could not split; a pipe inside a capability is unhandled. *Disposition: moot — step-01 §4 now takes a JSON file; no pipe-separated list remains.*
- [x] **[Review][Patch] `readContext`'s error sends `{scope_path}` failures to step-04 §1** [run-context.js:115] — wrong step; `initContext`'s JSDoc "Called once at step-04 §1" is false at the moment it was written. *Disposition: fixed — the message no longer names a step.*
- [x] **[Review][Patch] `writeAtomic` leaks an orphan `.tmp` on a partial write, has no data guard, and clobbers a pre-existing `.tmp`** [run-context.js:169]. *Disposition: filed as `T167`(c).*
- [x] **[Review][Patch] `recordContext(p,'__proto__',v)` silently records nothing; `undefined`/`NaN`/`Infinity` are silently dropped or nulled** [run-context.js:153]. *Disposition: filed as `T167`(a)/(b).*
- [x] **[Review][Patch] The story's Namespace decision and Project Structure Notes describe a file set the commit does not match** [story] — both say all work is under `_bmad/bme/`; the commit also adds `scripts/audit/`, `tests/audit/` and `.github/workflows/ci.yml`. The substantive rule is not breached; the stated justification is false. *Disposition: fixed — the Namespace decision was rewritten after Round 3; the Project Structure Notes still described the deleted gate until 2026-09-15.*
- [x] **[Review][Patch] AC#3's "not three separate tests" is unmet** [spec-parser.test.js, config-creator.test.js] — both still carry independent containment case lists; the shared table was added alongside rather than replacing them. *Disposition: fixed 2026-09-15 — the last separate list (`config-creator.test.js`, R3) was removed and its two unique values moved into `output-directory-cases.js`.*
...
Work spans `_bmad/bme/_team-factory/` and `tests/team-factory/`, plus comment-only citation fixes in `scripts/` and `docs/` and the VT wording in `docs/development.md`. The audit gate that once put files under `scripts/audit/`, `tests/audit/` and `.github/workflows/ci.yml` was deleted in Round 3. No `_bmad/` directory is renamed (BMAD Method compatibility). `no-process-cwd-in-libs` applies to every new function: take `projectRoot` explicitly, never fall back to `process.cwd()` — `config-creator.js::ensureOutputDirectory` already models this, rejecting a non-absolute `projectRoot` outright.
...
| name guard keyed on `=== undefined` (lets `null` through), or on key existence, or without `trim()` | *fails closed when a failing check has no usable name, rather than dropping it* — the first two survived until the `null` and `42` cases were added in the additional review |
...
**Correction (additional review, 2026-09-15): this walk was not in the documented order.** The spec was written with step-02 §6's `contracts` and `integration` already in it before step-02 §5's block ran — which is the only reason that block passed. In the documented order it throws `spec … is not usable: Missing required field: contracts; … integration`. The claim above that the flow was walked "as a contributor would" was false for step-02, and AC#1 was not met. The re-walk below is the evidence that replaces it.

**Re-walk in the documented order, after the additional-review fixes.** A driver extracted each `run:` line from the step files themselves and ran it through bash with only placeholders substituted — proving the blocks as written, not a retyping. Findings reproduced and closed on the way:

- step-01 §4's id check was fed `o'brien`, `a$(echo SUBST)b` and `x" ; echo INJECTED-BY-ID; echo "` → `{"valid":false,"bad":[…]}` with all three returned verbatim: no substitution, no execution. Corrected ids → `{"valid":true}`.
- The spec held only step-01's fields when step-02 began. step-02 §7's block, run where it used to sit (before §6 saves), threw the `Missing required field` error — the negative control. After §6 it returned `[]`.
- step-03's collision re-check had no scoping file, because step-01 says to delete it — `T176`, reproduced; the inventory was rebuilt from the spec to continue.
- §5d was given a **relative** `{registry_path}`; the context recorded it absolute and `PERSONA-COVERAGE` read it.
- `workflow_step_files` was recorded as the files under `steps/` only; the §8 manifest had 15 entries and no duplicates (`T175`'s reading).
- `validateTeam` → `valid: true`, 16 of 16. Removal as before: block-only registry diff (+39 −0) restored, SHA identical, `HEAD` unchanged, `git status --porcelain` identical to the pre-walk snapshot.
...
### Review Findings — operator-requested additional layers, 2026-09-15

Two read-only layers, at the operator's request after the story reached `review`: **A** scoped to `02b6d735` plus the CodeQL test edits `9c534e58`/`f5c4d607`; **B** a whole-story consumer audit — every caller of what this story changed, including callers no diff touched (the class that produced `T174`). Every HIGH below was reproduced before triage.

**AC#1 is not met at HEAD, and the Task 8 record overstated its walk.** The story returns to `in-progress`.

- [x] [Review][Decision] `[VT] Validate Team` can never pass — the `team-factory.md` menu, `module-help.csv` row 3 and `docs/development.md` ("available today") route straight to step-05, which has no context file and no `vortex_baseline` for a team step-04 did not just generate. Before this story it was impaired (T128 in a source tree) but could pass on an installed tree; now it cannot pass anywhere. `PERSONA-COVERAGE` also looks for `<PREFIX>_AGENTS`, which hand-built modules (`AGENTS`, `EXTRA_BME_AGENTS`) do not use. Options: narrow VT to teams generated by step-04 and say so on all three surfaces; withdraw VT from the menu, CSV and docs until it can work; or give VT its own mode that skips the differential (new code, new scope). **Ruled 2026-09-15: narrow.** The menu item, `module-help.csv` and `docs/development.md` now say VT validates a team `add-team` generated in this repository from that run's context file; step-05's Prerequisites say why it cannot validate a hand-built team.
- [x] [Review][Patch] step-02 §5's config-collision block cannot run in the documented order — it calls `loadSpec`, which validates the whole spec, and `integration`/`contracts`/`contract_prefix` are only written by §6, after it. Reproduced on a step-01-shaped Sequential spec: `spec … is not usable: Missing required field: contracts; … integration`. Same ordering trap the story fixed in step-01 §4, one step later [`step-02-connect.md` §5/§6]
- [x] [Review][Patch] step-01 §3's naming block interpolates the raw contributor id into a `node -e` string — the block that enforces the id regex is itself the first command the raw id reaches. A literal paste of `'a$(echo SUBST)b'` returns `"id":"aSUBSTb"` (the shell ran the substitution); `o'brien` is a syntax error. The Placeholders row claiming `{agent_id}` is safe "only because §3 enforces" the regex is false [`step-01-scope.md` §3, Placeholders]
- [x] [Review][Patch] Task 8's record says the walk pasted the blocks "as a contributor would", but the spec carried step-02 §6's fields before step-02 §5 ran — which is why that block passed. Correct the record [this file, Task 8 notes]
- [x] [Review][Patch] §5d's `expect:` lists `success === false → attempt rollback` above `dirty === true`; a dirty result also has `success: false`, so it matches the rollback branch first — and the only restore command in the flow, `git checkout --`, discards the operator's uncommitted registry edits the dirty check exists to protect. Order was correct before `02b6d735` [`step-04-generate.md` §5d]
- [x] [Review][Patch] The unnamed-check test does not cover a `null` name, the one non-string that survives the context file's JSON round trip; a guard keyed on `=== undefined` passes it, and a real baseline→post sequence then reports "no Vortex check regressed" [`end-to-end-validator.test.js`]
- [x] [Review][Patch] step-05's PERSONA-COVERAGE recovery says "two causes"; a third — an agent file whose basename is not the agent id — yields `empty` with no issues and passes `AGENT-FILE-EXISTS`, so neither diagnosis matches. §5d and the "Pass `agentFiles`" paragraph also claim non-empty `agentFilesIssues`/`missingAgentFiles` mean the team is hollow; with good files alongside a stale entry the gate is green [`step-05-validate.md` §2, `step-04-generate.md` §5d]
- [x] [Review][Patch] §5d records `'{registry_path}'` raw; substituted as a relative path (as its neighbours `{spec_path}`/`{context_path}` are) a correct run goes red on `must be an absolute path` [`step-04-generate.md` §5d]
- [x] [Review][Patch] `{generation_context}` survives after the rename — in `checkVortexRegression`'s user-facing failure text, `step-05-validate.md` and `step-04-generate.md`; the `RegistryResult` typedef does not list `personaCoverage` [`end-to-end-validator.js`, `lib/types/factory-types.js`, step files]
- [x] [Review][Patch] Line citations this story's own edits broke: `step-04-generate.md` cites `config-creator.js:26` (now elsewhere); `manifest-tracker.js` and `manifest-tracker.test.js` cite `registry-writer.js:42`/`:75`; `docs/vortex-step-01-round-split-scaffold.md` and backlog `T134` cite `step-01-scope.md` lines. Replace with symbol anchors [those files]
- [x] [Review][Patch] Two false comments: the happy-path test says its stub root "fails nothing" (it fails `Enhance module`, equal to the stated baseline); `output-directory.js` says `createConfig` converts throws to `{success:false}` (`buildConfigData` runs outside its `try`) [`end-to-end-validator.test.js`, `lib/utils/output-directory.js`]
- [x] [Review][Patch] The Round 2 (Tasks 1–3) review list carries 21 unchecked items with no disposition although several are fixed at HEAD and four were filed as `T167`–`T170`; record the disposition beside each [this file]
- [x] [Review][Defer] `runVortexValidation` reuses cached `validator.js`/`agent-registry.js` within one process — duplicate of `T173`; unreachable from the flow's separate-process blocks — deferred, already filed

Dismissed: `9c534e58` (Copilot Autofix) was a red commit, superseded 25 minutes later by `f5c4d607` — history only. The remaining template-string splices in the test helpers are the CodeQL shape but not exploitable (test-controlled inputs, `JSON.stringify` literals; probed with injection payloads) — no action.
...
- Additional review (2026-09-15): `workflows/add-team/step-01-scope.md` (§3 check moved to §4, over the file), `step-02-connect.md` (collision check after §6), `step-04-generate.md` (§5d order and path), `step-05-validate.md` (VT prerequisites, third cause), `agents/team-factory.md` + `module-help.csv` + `docs/development.md` (VT narrowed), `lib/validators/end-to-end-validator.js`, `lib/types/factory-types.js`, `lib/utils/output-directory.js`, `lib/manifest-tracker.js`, `tests/team-factory/{end-to-end-validator,config-creator,manifest-tracker}.test.js`, `tests/team-factory/output-directory-cases.js`; comment citations in `scripts/docs-audit.js`, `scripts/audit/lib/installed-tree.js`, `docs/vortex-step-01-round-split-scaffold.md`
...
| Date | Note |
|---|---|
| 2026-09-15 | **Done, by operator decision after the additional review.** Origin rows closed in the backlog: `T136`, `T128`, `T164`, and part (a) of `T163` — (b) stays open, re-measured live and rescored. Closing notes in the completed-work archive. The epic closes with this story: its definition of done is what the Task 8 re-walk demonstrated. No further review round, as recommended and accepted. |
| 2026-09-15 | **Additional review applied; back to `review`.** Two operator-requested layers found AC#1 unmet at `review`: step-02 §5 could not run in the documented order, step-01 §3 ran the contributor's id through a shell, and the Task 8 record overstated its walk. Also a §5d branch order that could lead an operator to discard uncommitted registry edits. All fixed; VT narrowed by ruling. The evidence is a re-walk of every `run:` block, extracted from the step files, in the documented order, with hostile ids — `valid: true`, 16 of 16, tree restored to its snapshot. These fixes are unreviewed by construction; no further round was run. |
