...
>
> **State correction — SUPERSEDED 2026-08-13, retained for the record.** This banner claimed *"Tasks 3–5 shipped 2026-05-31 … Remaining work is Tasks 6, 7, 8"* and that the recordings were *"on disk and still valid"*. **Both halves were wrong**, and the error is instructive: Tasks 3/5 shipped only **Prompt 1** (25% of each recording; Prompts 2-4 are 87-byte placeholders), and the corpus is **not valid** — the Path B+ control agent `stack-detective` has byte-identical source across the two recorded commits yet produced differing recordings, so agent run-to-run variance is uncontrolled and confounded with migration effect. Inferring completion from file existence is exactly what this banner did. See the release record.
>
...
- [ ] ⚠️ **PARTIALLY EXECUTED 2026-05-31 (Prompt 1 only, 4 agents × 2 phases; Prompts 2-4 never captured). NOT waived — attempted and unfinished. Corrected 2026-08-13: the earlier 'WAIVED' label misdescribed real work as a decision.** **Task 3: Capture 4 baseline recordings from 3.x state (HALT for operator action).** (Path B+ scope — Decision 4 addendum, was 5 agents)
...
- [ ] **Task 4: Trigger v4.0 migration on sandbox (HALT for operator action).**
  - [ ] 4.1 ⚠️ **NOT PERFORMED / NOT DECLARED (found by review 2026-08-13).** Task 4 was never executed, yet `post_migration_commit: e8676ffe` — Task 4.3's deliverable — is published in the release record, back-derived from recording provenance rather than this task. Task 4.2's ship-blocking `convoke-version.js` verification never ran. Operator transitions sandbox to 4.0 state. **Worktree path:** `cd <main-checkout-path>` + `git worktree remove ../convoke-3x`. **Stash path:** `git checkout main && git stash pop` (resolve any conflicts manually).
...
- [ ] ⚠️ **PARTIALLY EXECUTED 2026-05-31 (Prompt 1 only). NOT waived — see Task 3. Corrected 2026-08-13.** **Task 5: Capture 4 post-migration recordings from 4.0 state (HALT for operator action).** (Path B+ scope — Decision 4 addendum, was 5 agents)
...
- [ ] ⏸️ **NOT RUN — battery never executed (inputs 25% complete). Disposition unresolved; see release record §Decision required.** **Task 6: Run battery harness (Story 4.2's script).**
...
- **2026-08-13 — Release-record v1 WITHDRAWN after 3-layer adversarial review; user-facing equivalence claims corrected; story returned to `in-progress`.** `/bmad-code-review` (Blind Hunter + Edge Case Hunter + Acceptance Auditor, waiver decision explicitly in scope) found the 2026-08-12 closure unsound. **Decisive finding — the corpus proves nothing:** the Path B+ control agent `stack-detective` has **byte-identical source** across the two recorded commits (`git diff 90bf3115 e8676ffe -- '*stack-detective*'` → empty) yet its baseline and post recordings differ substantially. With one capture per phase and no repeat-capture control, agent run-to-run variance is unmeasured and demonstrably as large as the migration effect — so the corpus supports **no** equivalence claim, and equally not the opposite. v1's *"agents activate correctly — supported"* is withdrawn; so is the intermediate correction that read the same differences as evidence of drift. **Second finding — the real evidence had zero weight anyway:** `computeGate` consumes `medianOf([P1,P2,P3,P4])`, which averages the two middle values, so with three placeholder 5s the median is **5 for every possible P1** (verified by execution across P1∈{1..5} → PASS avg 5.00 each time). **Third — v1's central justification was false:** *"battery cannot be scoped to Prompt 1 without editing frozen code"* — `loadAgentRecordings`, `runJudgePairs`, `computeGate`, `writeResults` are all public exports; AC7 forbids *modifying* the battery, not *consuming* it. **Fourth — the forbidden claim had already shipped:** `CHANGELOG.md:18`/`:24` (streamed to every upgrading user by `convoke-update`), `docs/migration/3.x-to-4.0.md:42`, and `convoke-announcement-4.0-draft.md:22` all asserted behavioural-equivalence testing; v1 routed the guard to Story 5B.1, closed 2026-04-27. **All four surfaces corrected**, with a durable claim-guard pinned in the announcement draft (the canonical voice source the CHANGELOG derives from). **Also corrected:** `install-scope-check.js` cited-but-never-run → executed (`✓ PASS`, exit 0) and quoted; Tasks 3/5 mislabelled "WAIVED" → **partially executed at 25%**; Task 4 unchecked *and* undeclared while its `post_migration_commit` output was published; `npm test` figure `1511/0` does not reproduce → actual **1485/1483/0/2**; "24 of 32 judged pairs" → **16 pairs / 48 calls**; the "~6 hr **plus sandbox rebuild**" price was inflated — the v3.3.0 worktree still exists at `/Users/amalikamriou/convoke-3x`. **Operator's binding PROVISIONAL condition (2026-08-11) restored** as structured frontmatter after v1 dropped it and the Change Log argued it away. **Sign-off removed:** review found signing a "waived" verdict *evaded* Task 7.4's guard rather than honouring it — AC4 defines the verdict domain exhaustively, EO-8 abolished operator-discretionary ship-with-caveats, and INVESTIGATE (a *measured* adverse outcome) is barred from sign-off, so no-measurement is barred more strongly. **Story → `in-progress`**, blocked on an operator decision; `review` asserted "implementation complete", which is untrue. **Unresolved and recorded, not fixed:** Story 4.5 Task 0.1 requires `m9_pf1_gate: PASS`; Story 5B.3 gate-1 requires 4.3 `done`; and the fabricated-PASS trigger is still loaded because `parseRecording` rejects only *empty* sections while the placeholder is 87 bytes.
- **2026-08-12 — M9 PF1 gate WAIVED at partial scope by operator decision; Story 4.3 closes without running the battery.** Resumed via `/bmad-dev-story` (Amelia) after Winston's staleness pre-flight. Task 0 pre-flight re-run: 0.1 ✅ (4.1 + 4.2 `done`), 0.2 ✅ (battery exports exactly 14), 0.3 ❌ *expected* (clean-slate gate is unsatisfiable on resumption), 0.4 ✅ (`v3.3.0` = `90bf3115`), 0.5(a) ✅ **passes as operationally defined** (Epics 1A/2/3 all `done`; 4.1/4.2 `done`; 4.4 `done`; 4.5 status checked) — the "23/29 stories" shortfall is Epic **1B**, deliberately deferred to 4.0.1 under Path A, so the operator override obtained beforehand was belt-and-braces rather than necessary; 0.5(b) ✅ `4.0.0-rc.1`; 0.5(c) ✅ migration present. **Blocking discovery at Task 6 input-verification: the recordings are 25% complete.** All 8 files carry the literal placeholder `_(D2-B operator fill-in required — see recording-protocol.md §6.1 for manual workflow)_` for Prompts 2, 3 and 4 — byte-identical between baseline and post. Running the battery would have scored **24 of 32 judged pairs** as perfect equivalence on content never recorded, producing a **fabricated PASS on a release-blocking gate**. This is the tooling behaving as designed (`pf1-record-agent.js:168` documents the v0 Prompt-1-only limitation) and Decision 4 priced the manual half at ~2-3 hr/phase — that work was simply never performed. Structural checks all passed (8/8 parse 4/4 keys, provenance present, baseline↔post prompt sequences identical), which is exactly why this was found by reading content rather than by validation. **Operator chose to waive** rather than spend the ~6 hr or modify the shipped battery. Release record authored + signed: `v63-4-3-release-record-4.0.md`, `m9_pf1_gate: WAIVED-PARTIAL-SCOPE`, scope confirmed as activation-level equivalence, residual risk mapped to MO7 classes (③ and ④ uncovered). **Tasks 3, 5 and 6 remain UNCHECKED by design — waived is not done.** Gates green: `npm test` **1485 tests / 1483 pass / 0 fail / 2 skip** (the `1511/0` figure recorded on 2026-08-12 does not reproduce and was withdrawn 2026-08-13), `test:integration` exit 0, `lint` exit 0.
