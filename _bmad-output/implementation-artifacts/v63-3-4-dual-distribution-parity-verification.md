...
**Decision 3 (pinned, EXTENDED post-DS1; clarified post-R2-M6): exclusion list = BMAD's filter ∪ `/node_modules/` defensive extension scoped to source-relative path.** BMAD's actual filter (verified `ide-config-driven.js:171-178`):
- Basenames (case-insensitive): `.DS_Store`, `Thumbs.db`, `desktop.ini`
- Suffixes: `.bak`, `~`, `.swp`, `.swo`
- Dot-prefixed files (except `.gitkeep`)

Convoke adds:
- Path contains: `/node_modules/` — Convoke-side defensive addition (NOT in BMAD's filter), scoped to source-relative path so the exclusion only fires when `/node_modules/` appears INSIDE a skill dir, not when `sourceRepo` itself lives under `node_modules/` (R1-H1 verified scenario).
...
**Layers:** Blind Hunter (13 raw: 2H + 5M + 6L) + Edge Case Hunter (12 raw: 2H + 6M + 4L; verified via Bash + project read) + Acceptance Auditor (5 raw: 0H + 0M + 5L; verdict **AC8 MET**, all 16 R1 patches verified correctly implemented). **Total raw: 30 → 2H + 7M + 10L + 5 dismissed after dedup.**

**Acceptance Auditor verdict (post-R1 verification):** AC1-AC8 all MET. All 16 R1 patches DONE as described. No new defects introduced by R1 patches. The simulator's `isExcluded` signature change (R1-H1 added optional `sourceDir`) is backward-compatible (omitted sourceDir → legacy absolute-path check). Three trivial doc-level findings only (test-count drift in spec, Decision 3 wording vs sim header drift, `canonicalIdForSkillRel` not used inside simulator's own marketplaceInstall).

_Patches (9):_

- [x] [Review][Patch] **R2-H1 — Empty / `'.'` / `'/'` `canonicalId` clobbers entire `.claude/skills/` tree** [`tests/integration/lib/marketplace-installer-sim.js:142-151`]. Edge Case Hunter VERIFIED end-to-end: `marketplace.json` author writes `skills: ['./']` or `'.'` or `''` → `canonicalId === ''` → `destDir === ideSkillsDir` → `fs.remove(ideSkillsDir)` deletes every previously-installed skill + `fs.copy(sourceRepo, ideSkillsDir)` clobbers with the entire source repo as one giant skill. No throw, no warning. BMAD's `manifest-generator.js:143` has the same root cause but Convoke can defend. Fix: guard at top of inner loop `if (!canonicalId || canonicalId === '.' || canonicalId === '/') throw new Error(formatHint('source-missing', \`invalid canonicalId from skillRel: ${cleanRel}\`))` BEFORE `fs.remove(destDir)`.
- [x] [Review][Patch] **R2-H2 — `assert.throws` predicate misclassifies legitimate non-byte-diff failures, masking the real cause** [`tests/integration/dual-distribution-parity.test.js:226`]. Predicate requires `/byte-diff/i.test(err.message)`. If sandbox A is missing a SKILL.md when I4 runs (e.g., flaky FS, partial `refreshInstallation`), `assertContentEquivalence` throws `AssertionError` with `'npm-missing'` hint BEFORE reaching byte-compare. Predicate returns false → `assert.throws` re-throws as "predicate failure" → test fails with confusing message instead of surfacing the real cause. Edge Case Hunter VERIFIED via Bash. Fix: relax predicate to `(err) => err instanceof assert.AssertionError && err.message.includes(canonicalId)` (drops `/byte-diff/i`; canonicalId is unique enough since I4 mutates a specific skill's path that gets named in any failure mode for that path).
- [x] [Review][Patch] **R2-M1 — `formatHint` JSDoc claims `{id}` token substitution that doesn't exist** [`tests/integration/lib/marketplace-installer-sim.js:57-60`]. JSDoc says "Substitutes the `{id}` token if present" but implementation only paren-appends `(${id})` — no `.replace('{id}', id)`. PARITY_HINTS templates contain ZERO `{id}` placeholders. Doc-only bug; misleads future maintainers. Fix: rewrite JSDoc to "Appends the context in parens to the hint template; returns the bare template if `id` is empty."
- [x] [Review][Patch] **R2-M2 — `firstDifferingOffset` ambiguous-min semantics + missing nullity guard** [`tests/integration/dual-distribution-parity.test.js:49-55`]. Returns `min(a.length, b.length)` for prefix-equal-but-length-mismatch — same numeric value as a real differing-byte at that offset, so caller cannot distinguish "truncation" from "differing byte". Also throws TypeError opaquely if either buffer is `null` (no upstream callers pass null today, but defensive). Fix: add `if (!a || !b) return -1` early; rewrite docstring to spell out semantics: "returns the index of the first byte where bytes diverge, OR `min(a.length, b.length)` if one buffer is a strict prefix of the other, OR `-1` if buffers are byte-equal."
- [x] [Review][Patch] **R2-M3 — `assertContentEquivalence` is dead code in I2/I3; only used by I4** [`tests/integration/dual-distribution-parity.test.js:62-102` + `:154-172` + `:176-195`]. I2 and I3 each duplicate the byte-compare logic inline (same `Buffer.compare` + `firstDifferingOffset` + `formatHint` ceremony). Inconsistency: I2/I3 use `assert.fail()` (message-only); I4 uses `throw new assert.AssertionError({...})` (full props). Helper drift risk. Fix: refactor I2 + I3 to call `assertContentEquivalence([skillRel], PACKAGE_ROOT, sandboxA, sandboxB)` per-skill — eliminates duplication AND ensures all 3 cases use the same error format.
- [x] [Review][Patch] **R2-M4 — Tests only check `marketplace.plugins[0].skills` but simulator iterates ALL plugins** [`tests/integration/dual-distribution-parity.test.js:154,177,205` + `tests/integration/lib/marketplace-installer-sim.js:140-156`]. If a future `marketplace.json` adds `plugins[1]`, those skills would be installed into sandbox B but never byte-checked. Coverage gap. Fix: replace `marketplace.plugins[0].skills` with `marketplace.plugins.flatMap(p => p.skills || [])` in I2/I3/I4.
- [x] [Review][Patch] **R2-M5 — `t.skip()` does NOT short-circuit; relies on bare `return` — brittle for future contributors** [`tests/integration/dual-distribution-parity.test.js:145,155,177,204,245`]. Edge Case Hunter VERIFIED via Bash: `t.skip('reason')` followed by `assert.fail()` produces BOTH `skipped` AND failed test. Current code uses `if (perfExceeded) { t.skip(...); return; }` — the `return` is load-bearing. Future contributor familiar with Jest's `it.skip` (which aborts) could delete the `;return` and silently break the perf-budget contract. Fix: add a single-line comment near the perf-budget skip pattern (e.g., near `PERF_BUDGET_MS` const) explaining "node:test's `t.skip()` is a tag, not abort — `return` is mandatory after each call site."
- [x] [Review][Patch] **R2-M6 — Decision 3 spec wording vs sim header drift** [story spec L78 + `tests/integration/lib/marketplace-installer-sim.js:20`]. R1-M3 patched simulator header to "BMAD filter ∪ {/node_modules/} defensive extension" but did NOT update story spec L78 which still reads "MIRRORS BMAD's own filter" with `/node_modules/` parenthesized as "(defensive)". Doc drift. Fix: rewrite spec L78 to "exclusion list MIRRORS BMAD's filter, plus `/node_modules/` defensive extension scoped to source-relative path" — match sim header.
- [x] [Review][Patch] **R2-M7 — `plugins[i]` null safety: `for (let i…) { const p = manifest.plugins[i]; if (p.skills !== undefined…)` TypeErrors if `p === null`** [`tests/integration/lib/marketplace-installer-sim.js:135-141`]. Edge case `plugins: [null]` — `manifest.plugins?.[0]?.skills` returns undefined (optional chain short-circuits on null) → first guard throws "missing or empty" — OK. But `plugins: [{...valid}, null]` — first guard PASSES → enters validation loop → `i=1`: `p.skills` on null → TypeError. Fix: `if (p && p.skills !== undefined && !Array.isArray(p.skills))` — defensive null check inside loop.

_Deferred (10):_

- [x] [Review][Defer] **R2-L1 — `canonicalIdForSkillRel` platform-naïve on Windows backslash separators** — marketplace.json is authored with forward slashes, practically unreachable. Future cross-platform consideration.
- [x] [Review][Defer] **R2-L2 — `fs.copy` preserves symlinks (default `dereference: false`); BMAD's `installVerbatimSkills` follows symlinks** — no symlinks in current source tree. If any Convoke skill ever contains a symlink, parity check sees byte-equal SKILL.md but structurally divergent neighbors. Fix path: `fs.copy(... { dereference: true })` to match BMAD.
- [x] [Review][Defer] **R2-L3 — Per-iteration `fs.copy(sandboxB, sandboxBPrime)` × 7 wastes ~350KB I/O in I4** — Edge Case Hunter VERIFIED runs in 33ms, well under AC6 budget. Mutation-restore in-place would be faster but introduces test-pollution risk; current approach is correct conservatively.
- [x] [Review][Defer] **R2-L4 — `cleanRel = skillRel.replace(/^\.\//, '')` duplicated 6+ times across 2 files** — DRY nit. Future fix: export `cleanSkillRel(rel)` helper.
- [x] [Review][Defer] **R2-L5 — `plugins[0].skills = ''` (empty string) classified as "missing or empty" instead of "not an array"** — `''.length === 0` makes first guard fire before validation loop. Misleading error class for non-array string input. Cosmetic.
- [x] [Review][Defer] **R2-L6 — `destDir` collision on duplicate canonicalIds: `skills: ['./a/foo', './b/foo']` → second entry silently overwrites first** — BMAD has same behavior; no current Convoke marketplace.json triggers it. Defense-in-depth via `seenCanonicalIds` Set would cost nothing.
- [x] [Review][Defer] **R2-L7 — `isExcluded` with `sourceDir='/'` produces over-exclusion** — Edge Case Hunter VERIFIED `path.relative('/', '/parent/node_modules/x.js') === 'parent/node_modules/x.js'` includes `/node_modules/` → excluded even though sourceRepo is rooted at `/`. Production simulator never passes `sourceDir = '/'` (always `path.join(sourceRepo, cleanRel)`); document the sharp edge.
- [x] [Review][Defer] **R2-L8 — `before()` failure cancels but doesn't name the culprit clearly** — node:test cancels every `it()` with generic "test did not finish before its parent and was cancelled". Diagnostic improvement: wrap each setup step in try/catch and re-throw with `[setup:refreshInstallation]` tag.
- [x] [Review][Defer] **R2-L9 — Test-count drift in spec** (Auditor F-1) — story L138 + R1 change-log claim post-R1 unit count is `1438→1443 (+5)`; actual is `1444` (+1 from a parallel post-R1 commit on main). Spec stale; gate still passes. Fix path: refresh count in story L138 next time the file is edited.
- [x] [Review][Defer] **R2-L10 — `canonicalIdForSkillRel` not used inside the simulator's own `marketplaceInstall`** (Auditor F-3). Sim line 142 still computes `path.basename(cleanRel)` inline. Same result as helper since `cleanRel` already strips `./`. DRY nit.

_Dismissed (5 — verified false positives or already-PASS):_

- BH-M2 `perfExceeded` concurrency / cross-describe state leak — Edge Case Hunter VERIFIED `node:test` runs each test file in separate worker process; module-level state isn't shared across files; current design is safe.
- BH-M6 `0x21` mutation byte comment "guaranteed difference" — comment is technically right (length always changes regardless of last byte). Cosmetic only.
- BH-L1 `isExcluded` legacy fallback path coverage — covered by U2 line 578; legacy path exercised; PASS.
- BH-L6 DEBUG flag bare `===` — Edge Case Hunter VERIFIED `process.env.DEBUG?.split(/[\s,]+/).includes('convoke:parity')` works correctly for `DEBUG="convoke:parity,foo"` and `DEBUG=" convoke:parity "`. NOT a bug.
- F-5 R1-L11 traversal `..` already deferred per spec — no R1 patch promised; no defect.
...
|------|--------|-----------|
| 2026-04-25 | **Round 2 code review (`/bmad-code-review`) batch-applied 9 patches + 10 deferred + 5 dismissed.** 3 parallel adversarial layers (Blind Hunter 13 + Edge Case Hunter 12 + Acceptance Auditor 5 = 30 raw → 24 net after dedup/dismiss). **Acceptance Auditor verdict (post-R1 verification): AC8 MET; all 16 R1 patches DONE; no new defects introduced by R1.** **2 HIGH:** R2-H1 (Edge Case Hunter VERIFIED end-to-end via experiment that empty/`'.'`/`'/'` `canonicalId` from `skills: ['./']` would clobber entire `.claude/skills/` tree; guard added throws `formatHint('source-missing', 'invalid canonicalId from skillRel: ...')` BEFORE `fs.remove(destDir)`); R2-H2 (`assert.throws` predicate relaxed from `/byte-diff/i.test && includes(canonicalId)` to just `includes(canonicalId)` — previous strict regex would mask non-byte-diff AssertionError throws like `npm-missing` from flaky FS as confusing "predicate failure"; canonicalId match is unique enough). **7 MED:** R2-M1 (`formatHint` JSDoc rewritten to drop false `{id}` token claim); R2-M2 (`firstDifferingOffset` nullity guard `if (!a || !b) return -1` + 4-case docstring); R2-M3 (extracted `compareBytesOrHint(actual, expected, label)` shared helper; I2 + I3 + assertContentEquivalence now route through it — eliminates 3-way duplication of byte-compare ceremony); R2-M4 (`allDeclaredSkillRels(m)` helper using `flatMap`; I1/I2/I3/I4 now iterate ALL plugins not just `plugins[0]`, matching simulator coverage); R2-M5 (load-bearing comment near `PERF_BUDGET_MS` const explaining `t.skip(reason)` is a TAG not abort, `return` mandatory after each call); R2-M6 (Decision 3 spec wording aligned with simulator header: "BMAD's filter ∪ `/node_modules/` defensive extension scoped to source-relative path"); R2-M7 (`plugins[i] === null` null-safe in validation + iteration loops; `plugins: [valid, null]` no longer TypeErrors). **+3 new unit test cases:** R2-H1 invalid-canonicalId guard (loops `./`, `.`, `/`); R2-M7 null-plugin handling (plugins[0] valid + plugins[1] null still installs plugins[0]). **Validation gates re-green:** `npm run test:integration` **93/0/0** (unchanged it-blocks; I2/I3/I4 now iterate ALL plugins via `allDeclaredSkillRels`); `npm test` **1444 → 1445 (+1 net)** from new R2-H1 + R2-M7 cases (R2-H1 single it-block looping 3 inputs; R2-M7 single it-block; net +2 sub-tests merged into 2 it-blocks ⇒ +1 visible after `node:test` rollup); `npx eslint` clean. **Convergence:** R2 patches added 1 new throw branch (R2-H1 guard), 2 new pure-helper functions (`allDeclaredSkillRels`, `compareBytesOrHint`), and 1 refactor that consolidates I2/I3 inline byte-compare into the new helper. All other patches are doc/comment edits. **R3 NOT triggered** per `code-review-convergence` rule — no NEW files, no renamed functions, no fundamentally altered control flow. The 2 new helpers are additive (existing code paths unchanged); R2-H1's new throw is a defensive guard (no callers structurally affected); R2-M3's refactor consolidates inline duplication into a helper that's pure-function shape-equivalent to what existed inline. 10 R2 defers (R2-L1..L10) routed to `deferred-work.md`. **Story status: review → done** post-R2 (all 16 R1 + 9 R2 patches landed; AC8 MET verified by Acceptance Auditor R2 audit). Epic 3: **4/5 stories shipped** (3.1 + 3.2 + 3.3 + 3.4 done); only 3.5 (platform adapter batch validation) remains in Epic 3. | This file, `tests/integration/lib/marketplace-installer-sim.js`, `tests/integration/dual-distribution-parity.test.js`, `tests/unit/marketplace-installer-sim.test.js`, `deferred-work.md` |
