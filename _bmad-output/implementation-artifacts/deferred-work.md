...
## Deferred from: code review of story i97-2-1 (2026-05-02)

- **CR-i97-2-1-D01 (Blind+Edge MED) — `parity-harness.js` `extractV5MenuCodes` multi-`<menu>` and missing-`<menu>` edge cases** [`parity-harness.js:1045-1054`] — Comment claims multi-menu extraction; `match()` lacks `g` flag returns first only. Missing-`<menu>` returns `[]` silently. Defer reason: Story 3.2 productionization owns harness hardening.
- **CR-i97-2-1-D02 (Blind MED) — `parity-harness.js` format detection over-strips fences for v5 SKILL.md** [`parity-harness.js:941`] — `stripCodeRegions(rawContent)` before `<agent>` test misses fenced canonical structure for un-converted agents. Defer reason: only impacts un-converted agents during 2.2-2.7 window; Story 3.2 owns.
- **CR-i97-2-1-D03 (Blind LOW) — `agentRoleName` regex restrictive whitelist** [`parity-harness.js:862`] — No underscore/uppercase. Current corpus fine. Defer reason: future-proofing not justified now.
- **CR-i97-2-1-D04 (Blind LOW) — `runParityCheck` "not-yet-converted" label ambiguous** [`parity-harness.js:946-1005`] — Overloaded for missing-file vs unrecognized-format. Defer reason: Story 3.2 owns CI gate productionization where label disambiguation matters.
- **CR-i97-2-1-D05 (Blind LOW) — `stripCodeRegions` tab-indented blocks edge case** [`parity-harness.js:1149-1167`] — `prevBlank` flag mishandling for indented-after-blank patterns. Defer reason: rare in current corpus.
- **CR-i97-2-1-D06 (Blind+Edge MED) — `module-help.csv` LF line endings + empty `menu-code` column** [`module-help.csv`] — RFC-4180 wants CRLF; menu-code field empty for an agent with 8 actual codes. Defer reason: schema choice (BMM canonical, agent-rollup row) is per AC8; CSV will be enriched as more agents added.
- **CR-i97-2-1-D07 (Blind LOW) — Sprint-status flipped to `review` in same workflow as authorship** [`sprint-status.yaml`] — Process concern: status flips ideally triggered by review workflow, not authorship. Defer reason: project-wide pattern, not Story 2.1-specific.
- **CR-i97-2-1-D08 (Blind+Edge MED) — `validate-marketplace.test.js` dual-name accept (legacy OR BMB canonical) + 3 sources of truth for canonical name** [`validate-marketplace.test.js:325-360`] — Test accepts both names; registry/manifest/test diverge. Defer reason: architectural during 2.1-2.7 migration window per ADR-001 hybrid naming; tighten at Story 2.7 close.
- **CR-i97-2-1-D09 (Blind LOW) — `runParityCheck` input handling asymmetry between v5/v63 branches** [`parity-harness.js:953,965`] — v5 branch passes raw content; v63 strips fences again. Defer reason: Story 3.2 productionization.
- **CR-i97-2-1-D10 (Edge MED) — `extractV63MenuCodes` regex matches `Code` header column with separator-line variations** [`parity-harness.js:1100-1103`] — Header-column variant edge cases. Defer reason: Story 3.2 productionization.
- **CR-i97-2-1-D11 (Edge MED) — `vortex-parity.test.js` `JSON.parse` outside `it()` blocks hides per-test failure on fixture corruption** [`vortex-parity.test.js:49`] — Suite-level throw masks specific failure. Defer reason: Story 3.2 test-pattern hardening.
- **CR-i97-2-1-D12 (Edge MED) — V63 menu-code regex over-matches continuation rows / `<br>` table cells** [`parity-harness.js:873`] — Future multi-line table cells could yield phantom codes. Defer reason: Story 3.2.
- **CR-i97-2-1-D13 (Edge MED) — `references/validate-context.md` workflow path not validated by capability-prompt validator** [`validate-context.md`] — Capability prompt's `## Activation` workflow path is not in `fixture.menuCodeToWorkflow` map and not separately validated. Defer reason: enhancement; FR12 invariant + parity test cover the main cases.
- **CR-i97-2-1-D14 (Edge MED) — `runParityCheck` `tmpDir` contract not honored** [`parity-harness.js:894-897`] — JSDoc says accepted, code uses live `PROJECT_ROOT`. Defer reason: Story 3.2 productionization.
- **CR-i97-2-1-D15 (Edge LOW) — `extractV63MenuCodes` strips fences before finding `## Capabilities` heading** [`parity-harness.js:1079-1085`] — Fenced-Capabilities-heading edge case (rare during conversion drafts). Defer reason: rare.
...
## Deferred from: code review of bug-high2-migration-backup-rollback-scope (2026-08-09)

- **CR-bug8-D01 (Edge LOW) — restore removes destPath before copy (data-loss window)** [`scripts/update/lib/backup-manager.js:190`] — `restoreBackup` does `fs.remove(destPath)` before `fs.copy`; a mid-copy failure leaves the original gone. Pre-existing pattern (the original restore did the same); best-effort restore widens the window since it now continues past a failure. Defer reason: pre-existing behavior, not introduced by BUG-8. Fix path: copy-to-temp-then-rename, or copy-before-remove.
- **CR-bug8-D02 (Edge LOW) — containment/delete guards are lexical, not realpath** [`scripts/update/lib/backup-manager.js:159`] — a symlinked `_bmad/_memory` (or intermediate dir) passes the `startsWith` containment + delete guards lexically, so `fs.remove`/`fs.copy` operate on the real out-of-tree target. Defer reason: exotic for a normal install (`_bmad/` is not symlinked); realpath adds I/O per entry. Fix path: `fs.realpath` before the containment and delete checks.
- **CR-bug8-D03 (Edge LOW) — restore/delete class conflict for the same relPath across a migration chain is order-dependent** [`scripts/update/lib/migration-runner.js:88`] — the runner `flatMap`s each migration's `getBackupManifest` with no dedup; two chained migrations declaring the same path as restore vs delete produce order-dependent results. Defer reason: not reachable today (only `3.3.x-to-4.0.0` implements the hook). Fix path: dedup entries + resolve restore/delete conflicts deterministically (or error) when a second migration adopts the hook.

## Deferred from: code review of bug-high2-migration-backup-rollback-scope — Round 2 (2026-08-09)

- **CR-bug8-D04 (Edge/Blind MED→deferred) — containment/delete guards are lexical, not realpath** [`scripts/update/lib/backup-manager.js`] — `_resolveContained` and the `_bmad/_memory` delete guard validate the *lexical* resolved path; a symlinked ancestor (`_bmad` or `_bmad/_memory` pointing outside the tree) would pass containment but `fs.copy`/`fs.remove` follow the link at runtime. Deferred: abnormal precondition (Convoke installs don't symlink `_bmad/`), and `fs.realpath` per entry adds I/O. Fix path: `fs.realpathSync` the existing parent dir and re-check containment before copy/remove, or `lstat`-reject symlinked ancestors.
- **CR-bug8-D05 (Edge LOW) — AC6 integration test depends on `npx convoke-doctor`** [`tests/unit/migration-runner-rollback-bug8.test.js`] — `_phase2/_phase5` call `_runDoctor` → `execFileSync('npx', ['-p','convoke-agents','convoke-doctor'])`, which throws on ENOENT/timeout in an offline/network-less CI, aborting `apply()` before phase 3 rewrites the stub → the AC6 assertion could pass vacuously (the sibling non-vacuity guard test turns this into a *visible* red instead, so it is not silent). Deferred: proper fix needs an injectable doctor-runner seam (`_runDoctor` is a module-local call, not stubbable via `_internal`). Fix path: thread a doctor-runner dependency into `_phase2/_phase5` (or export+inject) so phases 2/5 don't require the network.
- **CR-bug8-D06 (Auditor LOW) — no test for a `type:'directory'` dynamic entry or a multi-migration chain** [`tests/unit/backup-manager-rollback-scope.test.js`] — `_normalizeBackupEntries` carries a `type` field but only `file` entries are tested; and only one migration implements `getBackupManifest`, so the multi-migration `flatMap` merge (dedup/order — see D03) is untested. Deferred: both latent. Fix path: add a dir-type round-trip test and a two-migration-chain fixture when a second migration adopts the hook.

## Deferred from: code review of bug-high2-migration-backup-rollback-scope — Round 3 (2026-08-09)

- **CR-bug8-D07 (Edge MED, pre-existing — NOT a BUG-8 regression) — resume-after-hard-kill rollback restores already-rewritten (v4) content as "original"** [`scripts/update/lib/backup-manager.js` + the migration resume path] — if run #1 rewrites some `SKILL.md` + writes the state file, is hard-killed (SIGKILL/power-loss, so no rollback runs), then run #2 resumes: `getBackupManifest` backs up the *current on-disk* (already-rewritten) files, and if a later phase fails, `restoreBackup` copies that v4 content back as though it were the pre-migration original, leaving a mixed pre/post-v4 tree. The Round-2 abort only covers the `getBackupManifest`-*throws* case, not this success-path resume. This is the deferred **Story 1A.5 migration idempotency/resume** work — out of BUG-8's scope (BUG-8 fixed rollback *scope*, not resume snapshot semantics). Fix path: refuse to resume (or take a fresh persistent pre-migration snapshot) when a prior partial backup/state already exists.
- **CR-bug8-D08 (Edge LOW, latent) — delete-class rollback root is hardcoded to `_bmad/_memory/`** [`scripts/update/lib/backup-manager.js`] — a future migration that declares `onRollback:'delete'` for a file it creates *outside* `_bmad/_memory/` (allowed by the manifest contract) would have that file refused at restore ("delete target outside _memory") and never cleaned up. Not triggerable today (only `STATE_FILE_RELATIVE`, inside `_memory`, is delete-class). Fix path: carry an explicit per-entry `deleteRoot` (or allow any `_bmad/`-contained delete target) when a second delete-class use appears.

## Deferred from: code review (2026-08-10)

Round 1 adversarial review (Blind Hunter + Edge Case Hunter) of the CI green-restoration diff (`aed53068^..HEAD` + I124 working tree). Items below are real but pre-existing or out of scope for that diff.

- **`python-test` gates `publish` while verifying zero Convoke-owned code.** `.github/workflows/ci.yml:192` lists `python-test` in `needs:` for `publish`. Convoke owns only `_bmad/bme/`, which contains no `.py` files; the single discovered test (`_bmad/core/skills/bmad-distillator/.../test_analyze_sources.py`) is upstream-vendored BMAD code. So an upstream test Convoke neither owns nor reviews can block a Convoke release, while no Convoke Python is verified. Pre-existing — the job always scanned all of `_bmad/`; the enumeration change did not introduce it. Decide whether the job should scope to `_bmad/bme/` (and then likely be dropped, since there is nothing to test) or remain a broad upstream-conformance smoke gate.
- **Dynamic test discovery paired with a static dependency install.** `ci.yml:162` installs exactly `pytest pyyaml ruff` while the enumeration now collects an unbounded, upstream-controlled set of test files. The first upstream test importing a fourth dependency hard-fails a `publish`-gating job. Bundles with the item above — both are consequences of the job's scope being upstream-wide.
- **`js-yaml` floor raise has no CHANGELOG entry.** `package.json` `^4.2.0` → `^4.3.1` (GHSA-52cp-r559-cp3m / CVE-2026-59870). `^4.2.0` already admitted 4.3.1, so only the lockfile needed to move; raising the declared floor imposes a new minimum on consumers of a published package. Not wrong — it prevents a clean install resolving to a vulnerable 4.2.x — but it is a consumer-facing change that should be recorded at release time.

## Deferred from: code review Round 2 (2026-08-10)

Round 2 verified the Round 1 fixes and found 12 issues. The two HIGH (dead `find`-status guard; false `bmad-distillator` retirement claim) plus the false comments and dead `disabledSuites()` were fixed in-session. The items below are real but are design changes rather than patches, and per `code-review-convergence` there is no Round 4 — they go to the backlog instead of a third remediation pass.

- **CR-r2-D01 (MEDIUM) — disabled suites still vanish from test counts.** `describe(name, { skip })` suppresses subtests before they register, so `node --test` reports `tests 0 / skipped 0` for each of the 10 disabled portability suites — roughly 62 tests absent from the totals rather than counted as skipped. The delivered mitigation is a `console.warn` per suite, which no gate reads and which CI log truncation can discard. Round 1 asked for them to *register as skipped*; that needs a per-suite `it('…', { skip })` placeholder or `test.skip()` stubs — a pattern change across 10 files, not a patch. Bundles with **I123**, which deletes the guard entirely.
- **CR-r2-D02 (MEDIUM) — `suggestDifferentiator` is executed but unasserted.** `tests/lib/migrate-artifacts.test.js` now forces a real collision and asserts `collisionWith`, restoring `detectCollisions` coverage. But nothing asserts the *suggested* differentiator value; a regression returning an empty Map would leave `suggestedNewPath` null on every entry and the test would still pass. Line coverage without behavioural coverage. Fix with the fixture work in **I123/I125**.
- **CR-r2-D03 (MEDIUM) — the migrate-artifacts fixture killed `getContextClues`' git success branch.** The fixture lives under `os.tmpdir()`, which is not a git worktree, so `git log -1` fails for all 20 AMBIGUOUS entries inside a swallowing `catch {}`: `contextClues` present on 20 entries, `gitAuthor` non-null on 0. The author/date assignment path that the live-repo version did exercise is now permanently dead, and the suite still pays for 20 failing subprocess spawns. Either `git init` the fixture or assert the degraded shape explicitly. Bundles with **I125**.
- ~~**CR-r2-D04**~~ — ✅ **RESOLVED 2026-08-11.** Root cause found and fixed: `tests/unit/convoke-update-governance.test.js` wrote to `child.stdin` on a fixed 200ms timer with no check that the child was still alive. Fast paths — notably `--dry-run`, which does no work — exit in under 200ms, so the write hit a closed stdin and raised `EPIPE` as an **uncaught exception**, failing whichever test happened to be running. That is why it presented as load-dependent and unattributable. Fixed by skipping the write when the child has exited (`exitCode`/`signalCode`/`stdin.destroyed`) and treating `EPIPE` on stdin as benign (the child not needing the input is the scenario under test); any other stdin error still throws. Confirmed by CI run **31464560310** — all eight jobs green, `coverage` in 46s. **Caveat worth keeping:** one green run is not proof for a race — this test passed every run on 2026-08-10 and failed on 2026-08-11. Watch for recurrence; if it returns, the next suspect is the `child.kill()` timeout timer at `:52-55` racing the same write, which this fix does not cover. *The separate `manifest.test.js` slowness noted in the original entry was split out as backlog **I125** and remains open (local-only; CI is unaffected because `actions/checkout` uses a shallow clone).* Original entry follows for the record:
- **CR-r2-D04 (MEDIUM, pre-existing) — `npm test` is non-deterministic under load.** Round 2 measured `tests/lib` at 69s / 388s / >120s across three runs, with one run returning `fail 1` that did not reproduce, and observed an `ERR_ASSERTION actual: 1, expected: 0` at `tests/unit/convoke-update.test.js:425`. Neither file is in the reviewed diff. Largely explained by **I125** (`manifest.test.js`, ~16 min, unbounded), but the `convoke-update` assertion is separate and unattributed. Investigate after I125 lands, since I125 dominates the timing noise.
- **CR-r2-D05 (LOW) — `before(fn, 30000)` hook timeouts are silently ignored.** `portability-export-all.test.js:69`, `portability-full-pipeline.test.js:73`, `portability-seed-catalog.test.js:57` pass a numeric second argument to `before()`; `node:test` does not coerce it to `{ timeout }` and discards it. Harmless today because `if (SKIP) return;` short-circuits those hooks, but the moment the guard stops firing (post-I123) they resume spawning export/seed subprocesses unbounded. Fix as part of **I123**, when those hooks become live again.
- **CR-r2-D06 (LOW) — the guard hard-requires `git`.** `portability-preconditions.js` spawns `git ls-files` at module load of 11 test files with no fallback, so a missing `git` binary or a non-worktree checkout produces 11 separate hard failures instead of one diagnosable error. Also, `git ls-files` reports the index, so a tracked-but-deleted file reads as present — the inverse of the false-green being prevented. Accepted deliberately (git-tracking is the only oracle that answers "will this exist in a clean checkout?"), but the trade should be stated where the trap is documented.

## Deferred from: code review Round 2 — security remediation (2026-08-11)

Round 2 verified the RCE fixes and found the remediation had treated two bug *classes* as line-level bugs. The two HIGH items (lost `spawnSync` hardening; twin-file `icon` RCE in `registry-appender.js`) were fixed in `24b93df6`. The design-level response is backlog **I129**. The items below are real, verified by the reviewer, and deliberately not fixed inside a security commit.

- **CR-sec-D01 (MEDIUM) — `escapeSingleQuotes` throws on non-string input, escaping the writer's error contract.** `registry-writer.js` does `str.replace(...)` with no `String(str)` coercion, while `sanitizeForComment` — written in the same change — does coerce. A YAML spec with `icon: 5` or `title: 6.3` (ordinary YAML scalars) raises `TypeError: str.replace is not a function` out of `buildModuleBlock`, which is called at `:47` **outside every try** — so callers get a rejected promise instead of `{success:false, errors:[...]}`. Fix: coerce, and decide whether non-string scalars are valid spec input at all (validation belongs upstream of the writer).
- **CR-sec-D02 (MEDIUM) — `derivePrefix` strips rather than rejects, so all-punctuation team names silently collide.** `derivePrefix('!!!')` → `''` → `const _AGENTS = [` — valid JS, so nothing errors. Verified end-to-end: team `!!!` wrote `_AGENTS` and returned success; a later team `???` returned `{"success":true,"skipped":["block already exists"]}` with `written:[]` — its agents were never written and the caller was told it succeeded. The justifying comment I added ("keeps a malformed spec a validation error rather than a syntax break") is **false for the empty case**. Fix: reject an empty/invalid derived prefix explicitly; also reject digit-leading prefixes (`9-lives` → `const 9_LIVES_AGENTS`, currently surfacing as a raw V8 `SyntaxError` with a temp-file path rather than "invalid team name").
- **CR-sec-D03 (MEDIUM) — the injection regression suite covers 1 of 9 escaped fields, and `registry-appender.js` has none.** `registry-writer-injection.test.js` tests `icon` only. The bug class is precisely "one field among siblings was missed", so single-field coverage does not guard it. The reviewer confirmed by execution that the other 8 fields plus workflow names are *currently* safe (27 field × payload-shape combinations, three shapes incl. trailing-backslash and template-literal), so this is a coverage gap rather than a live hole. Fix with I129, or sooner: table-drive the existing test across every interpolated field in both writers.
- **CR-sec-D04 (LOW) — the test harness's documented error handling does not exist.** `registry-writer-injection.test.js:71` returns `{ status, stderr }` with no `res.error` or `status === null` branch, and `stderr` is destructured by zero call sites. On a spawn failure or the 5s timeout the assertion reports "expected null to equal 0" with no diagnostics. Fails safe rather than passing silently, but the description of the harness overstates it.
- **CR-sec-D05 (LOW) — `escapeSingleQuotes` and `sanitizeForComment` disagree about U+2028/U+2029.** The comment sanitizer strips them; the string escaper leaves them. Not exploitable on Node (ES2019 legalised both in string literals), but `agent-registry.js` is a persisted artifact that older parsers and bundlers reject. Pick one policy.
- **CR-sec-D06 (LOW) — `validateStaged` harvests identifiers from inside string literals.** `moduleBlock.matchAll(/const\s+(\w+)\s*=/g)` is a textual scan, so an agent with `title: 'const X = 1'` makes the additive-only check believe the block declares `X`, producing a spurious "Additive-only violation: X already exists". Denial of service on valid input, not a bypass. Dissolves under I129 (a structured builder knows its own declarations).
- **CR-sec-D07 (LOW) — injection-test temp directories leak.** `mkdtempSync` at module scope with no cleanup; `afterEach` removes only `PWNED*` files, leaving `blk-*.js` and the directory behind on every run.

## Deferred from: code review of fast-readme-rewrite-lifecycle-spine (2026-08-15)

- **CR-README-D01 — Installer banners still print the retired tagline.** `scripts/install-vortex-agents.js:26` and `scripts/install-gyre-agents.js:26` print "Agent teams for complex systems" at install time — a more user-facing surface than the npm description, and the exact artefact D1 retired. Outside this story's scope (`scripts/`, and touching it pulls in the test requirement).
- **CR-README-D02 — `package.json` `description` still carries the retired tagline.** npm package page and search results.
- ~~**CR-README-D03**~~ — ✅ **RESOLVED 2026-08-21** by story `dist-1-1` (ADR-001, option (a)): the generator, `docs/badges.json` and `.github/workflows/badges.yml` are deleted, and `badges`, `badges:check` and `prepublishOnly` are removed from `package.json`. The gate no longer exists, so there is nothing left to decide. Original entry follows for the record:
- **CR-README-D03 (archived — original text, no longer live) — `docs/badges.json` is an orphan that still gates `npm publish`.** After the badge cull nothing renders it, yet `badges:check` remains wired into `prepublishOnly`, so a drifted `badges.json` blocks a release for an artifact no document consumes. **Note:** the story's D9 justified keeping the machinery on the grounds that "the file has non-README consumers" — that claim is false; a repo-wide grep finds only `scripts/generate-badges-json.js`, `.github/workflows/badges.yml`, `knip.json` and `package.json`. Decide whether to retire the gate or give the file a consumer.
- **CR-README-D04 — `scripts/docs-audit.js` cannot see most of the docs' links.** `checkBrokenLinks` skips `^https?://`, so the 14 absolute URLs this story introduced leave CI validation entirely; and `CREDITS.md` is a newly shipped doc absent from `USER_FACING_DOCS`. Both belong to backlog **I157** (packed-tarball link gate), which should also resolve `github.com/amalik/convoke-agents/blob/main/<p>` back to a repo path.
- **CR-README-D05 — Absolute URLs hardcode `blob/main`.** A README published with 4.0.0 resolves against whatever `main` says months later; the `bmad-enhanced` → `convoke-agents` rename proves the path is not stable. A release tag would pin docs to the version the reader installed.
...
## Deferred from: code review of dist-1-1-retire-the-badges-pipeline (2026-08-21)

- **CR-dist11-D01 — Publish-gate window between `dist-1-1` and `dist-1-7`.** Removing `prepublishOnly` deletes the epic's only local publish chokepoint (`convoke-epic-4-0-1-distribution-integrity.md:81`), while its replacement — the registry-side setting accepted in ADR-003, FR9 — lands in `dist-1-7-make-the-ci-path-the-only-path-to-the-registry`, status `backlog`. T35 ("hand-publishing bypasses the entire CI publish gate") is still Open in the Fast Lane. Between this commit and `dist-1-7`, nothing local or remote stops a laptop `npm publish` from a dirty or unpushed tree. Deferred because the risk is owned by T35 and `dist-1-7`, and the removed gate was weak (it only diffed `badges.json`) — but the story carries no pointer to it, so a reader of `dist-1-1` alone will not see the window. Raised independently by two review layers.
...
## Deferred from: code review of dist-2-4 (2026-08-30)

- **ESM blindness in the bin dependency walk.** `scripts/audit/lib/installed-tree.js:234` regexes only `require(...)`. An `.mjs` bin, or any bin under `"type": "module"`, yields zero specifiers — and empty is the PASS value, reported to the operator inside the reassuring line "all 14 bins present, shipped, parseable, and their requires resolve". Same shape as the four fail-opens `try-fresh-install.sh` already records: absence of evidence read as evidence of absence. Already tracked as **T101(a)**, which sequences it first on the ground that it is the only T101 survivor able to report a broken package as healthy. Noted here because `dist-2-6` promotes this code into the publish-gating verdict while it is unfixed. *Reason for deferring: already filed and scored as T101(a); duplicating it as a story task would create the two-rows-one-defect pattern the backlog rules forbid.*
- **Fresh-install exclusion skew between the generator and the assertion.** `refresh-installation.js:48-54` reads `excluded_agents` from the TARGET module config before that config has been copied, so on a fresh install it reads `[]` and generates a wrapper for every agent; `installed-tree.js:314` reads the same key from the now-copied config and skips those agents. An operator shipping a non-empty `excluded_agents` would therefore have wrappers generated that the assertion stops asserting — a fail-open in the direction the check exists to prevent. Latent today: every shipped module config carries an empty or absent `excluded_agents`. *Reason for deferring: closing it correctly requires changing when the installer reads exclusions, which is generator behaviour outside this story's scope and touches the install path `dist-2-6` is about to modify.*


## Deferred from: T79 owed-close scan, first observed false positive (2026-08-30)

T79 (`backlog-integrity.js` owed-close scan) shipped 2026-08-27 in `dac6cb23` (`feat(T79): warn when a row's work has shipped but the row is still open`) and warned for the first time on 2026-08-30, against `I113`. Not filed as a row — the check is warn-level, it behaved as designed (it warned, a human read it, the human resolved it in seconds), and at this frequency a row would price near 1.5. Recorded so a later occurrence has something to file *from*.

- **An ID-scoped `governance()` commit that annotates a row is indistinguishable from one that implements it.** `fca5fcf7` and `0503f9b7` both carry scope `I113`; both are pointer edits to that row's Dependencies cell, and the second says so in its body (*"pointer only — no rescore, no reinstatement, MVP unchanged"*). T79's model is *a work commit naming row X means X's work shipped*, so it read two annotations as two implementations and warned that a close was owed on an initiative that is genuinely mid-flight. Verified against the committed tree in a detached worktree at HEAD, not the working copy.
...
- source_spec: `_bmad-output/implementation-artifacts/oc-2-1-retrofit-bottleneck-skills.md`
  summary: the retrofit creates 16 new operator-decision branches and none of them proposes a default, and it un-vacuums two cells A26 scored `N/A — vacuous` on the ground that step-01 had no decision branch at all.
  evidence: Round 1 Blind Hunter, verified. A26 §4.6 scored `production-monitoring` `Right to a default — N/A vacuous` because "step-01 is an intake form … with no conditional branching; no operator-decision-pause branch". The round split creates exactly that branch, and §A41-1 fires OC-R1 on all operator-decision branches with absence of a default-suggestion a FAIL. AC5's no-regression check looked only at whether the pre-existing fallback sentence survived, so it could not see branches the change itself introduced. Not fixed at the round cap: proposing a default at each new boundary is a design question about what a sensible default input even is, not a patch. *(qualified 2026-09-04 into **T122** — the backlog was clean, so the ID-allocation block lifted.)*

- source_spec: `_bmad-output/implementation-artifacts/oc-2-1-retrofit-bottleneck-skills.md`
  summary: AC6 cascade termination was evaluated only inside the 12 changed files, and removing the step-01 enumerations pushes concepts downstream into step-02 where they now arrive novel.
  evidence: Round 1 Blind Hunter, verified against source. `experiment-design/workflow.md` contains none of "assumption risk map", "testing order", "lethality" or "uncertainty", so those reached `steps/step-02-context.md` by inheritance from step-01's deleted §3; step-02 now introduces the 4-field table, Assumption Risk Map, Lethality, Uncertainty and the Test-First/Test-Soon framing in one round. Same shape at `assumption-mapping/steps/step-02-context.md`. The story's AC6 argument ("no round exceeds 3/3") is scoped to the changed files and does not test this; A24/A26 being step-01-scoped is the loophole. *(qualified 2026-09-04 into **T123**; the Round 2 acceptance audit returned AC6 FAIL on this.)*

- source_spec: `_bmad-output/implementation-artifacts/oc-2-1-retrofit-bottleneck-skills.md`
  summary: the scaffold and the tree have now diverged in four consecutive review rounds, and the sentence written at Round 2 to record the divergence is itself wrong.
  evidence: Round 3. `docs/vortex-step-01-round-split-scaffold.md:125` says two things "survive by wording, not by position", but its own row 2 says the `## Your Turn` prose is rewritten — that row survives by *heading*, i.e. by position. The section heading `## Must survive verbatim` is now false of two of its four rows. Its normative `## Your Turn` rule ("must NOT re-ask for the input round 1 already collected") also contradicts Round 2's own `behavior-analysis` fix, which deliberately restored an operator ask at that boundary; four shipped files legitimately collect input there. And line 61 still prints one fixed fallback lead-in outside angle brackets while five distinct lead-ins ship — the same instruction that produced Round 2's doubled-conditional defect. Not fixed: `code-review-convergence` allows no fourth pass, and the recurrence rate suggests the real answer is that a prose document cannot stay in sync by discipline alone.

- source_spec: `_bmad-output/implementation-artifacts/oc-2-1-retrofit-bottleneck-skills.md`
  summary: five step files count `readiness assessment` in a §3 whose body is identical to two other files that count only the schema validation — an AC2 divergence the pacing gate cannot see.
  evidence: Round 3, verified. `hypothesis-engineering`, `assumption-mapping`, `experiment-design`, `research-convergence` and `signal-interpretation` all footer `2/3 (schema validation, readiness assessment)` for a §3 consisting of the italic check line plus the contract pointer; `pivot-resynthesis` and `production-monitoring` footer `1/3` for the same construct. AC2 states "divergence between the 9 implementations is a defect". This is the same inflating-direction error Round 2 reverted in hypothesis-engineering's round-1 footer, corrected two lines above and left standing two lines below in the same edit. The gate's check 2 counts names, not whether the round introduces them, so it is structurally blind to this.

## Deferred from: code review of skill-manifest lint promotion (2026-09-05)

Fast Lane, no story file. The change promoted the sp-1-2 manifest lint out of
`tests/lib/portability-classification.test.js` into `scripts/audit/skill-manifest-integrity.js`.
All three items below are pre-existing or policy calls, not defects introduced by the move.

- source_spec: `scripts/audit/skill-manifest-integrity.js` (Fast Lane — no spec file)
  summary: the `>= 3` floor on `bmad-testarch-*` rows is a hardcoded magic number while the live manifest carries 8, so five rows could be lost in a bad merge with every gate staying green.
  evidence: Round 1 Blind Hunter, verified — `audit()` asserts `testarchRows.length < 3`, and the live tree has 8 matching rows. `derive-counts-from-source` targets exactly this shape. Carried over unchanged from the original test rather than introduced here. Not fixed at Round 1 because choosing the replacement is a policy question — deriving the floor from the tracked `bmad-testarch-*` skill dirs would change what the gate asserts, and the right floor is an operator call, not a patch.

- source_spec: `tests/audit/skill-manifest-integrity.test.js` (Fast Lane — no spec file)
  summary: **CLOSED 2026-09-09 by sp-7-1.** ~~the fixture `HEADER` is a 9-column literal with no linkage to the real manifest, so a column rename is not reproducible from the fixture suite.~~ `tests/audit/skill-manifest-integrity.test.js` now does `const HEADER = [...auditModule.EXPECTED_HEADER_COLUMNS]`, and `header/column-order` runs against the real manifest in CI. Note the evidence below cites `tests/lib/portability-schema.test.js`, which sp-7-1 deleted; the property it described is now enforced by the audit instead.
  evidence: Round 1 Blind Hunter. **Corrected at Round 2** — the original wording of this entry claimed a column rename would leave CI unable to name the column, and that reading the real header in a test would reintroduce live-tree coupling. Both halves were false: `tests/lib/portability-schema.test.js:63` already asserts the live header with `assert.deepEqual(headerCols, EXPECTED_HEADER_COLUMNS)` and would go red naming it, and that file demonstrates the coupling already exists elsewhere. The real residual gap is narrower than filed: the fixture suite cannot *reproduce* a header change, only detect it from the other file. Recorded rather than silently dropped, because a backlog row resting on a false premise sends the next reader down the same wrong path (`documentation-claims-must-be-derived`).

- source_spec: `tests/lib/portability-schema.test.js` (Fast Lane — no spec file)
  summary: **CLOSED 2026-09-09 by story sp-7-1.** ~~the `test-fixture-isolation` violation is NOT resolved as a class — a sibling test in the same directory still reads the live tree, and the classification vocabulary now exists in four independent copies.~~ `tests/lib/portability-schema.test.js` was deleted after its three unique assertions (header ORDER, exact row arity, schema-doc conformance) were folded into `scripts/audit/skill-manifest-integrity.js`; its tier/intent assertions were already covered by `row/invalid-tier`/`row/invalid-intent`. No file under `tests/` now reads the live manifest as data, and the vocabulary copies went from four to three. The remaining three are deliberate independence between checkers, not drift — see the pinning test.
  evidence: Round 2 Blind Hunter, verified. `tests/lib/portability-schema.test.js:51` calls `findProjectRoot()` and asserts against the live `_bmad/_config/skill-manifest.csv` — its exact header (`:65`), 9 columns per row (`:68`), and any NON-EMPTY tier/intent value against its own local `VALID_TIERS`/`VALID_INTENTS` (`:76`, `:90`) — which is the same violation the promotion of `portability-classification.test.js` was written to fix. Separately, `VALID_TIERS` is declared independently in `classify-skills.js:35`, `validate-classification.js:44`, `portability-schema.test.js:18` and the new audit script; Round 2 rejected importing it from `classify-skills.js` because a checker must not take its vocabulary from the writer it polices (proven: widening the writer made the audit accept a bogus tier with zero test failures), so the audit declares its own and a test pins the writer/checker pair. Deliberately NOT fixed here: the operator chose to keep this Fast Lane change at its original size rather than let a one-file fix grow into a governance refactor. Closing it properly means promoting the sibling test and giving the vocabulary a single home — one coherent story, with a spec, not a third patch pass.

- source_spec: `scripts/portability/classify-skills.js` (Fast Lane — no spec file)
  summary: `classify-skills.js` holds a per-persona intent policy (`PERSONA_AGENT_INTENTS`: tech-writer → `write-documentation`, dev → `plan-your-work`) that no checker verifies; the audit pins persona *tier* and *deps* but never *intent*.
  evidence: Round 1 Blind Hunter, verified against `scripts/portability/classify-skills.js:87-97`. Surfaced because the test fixture assigns every persona `define-what-to-build`, which contradicts the writer on two rows and is harmless only because the audit does not check that axis. Genuinely pre-existing — the original test did not check persona intent either. Not fixed because closing it means deciding whether the writer's per-persona table is normative policy or an implementation heuristic, which is an operator ruling, not a patch.

- source_spec: `scripts/audit/skill-manifest-integrity.js` (Fast Lane — no spec file)
  summary: the `manifest-gap` retirement guard can only fire for the minority of manifest names that have a TRACKED `SKILL.md`; most installed skills live in gitignored `.claude/skills/`, so for them a retirement is rubber-stamped unchecked.
  evidence: Round 3 Blind Hunter, verified. `trackedSkillDirsAt` enumerates `git ls-files`, and `.gitignore:59-64` ignores `.claude/skills/` except two un-ignored directories by name. So of 106 manifest rows, only those with a tracked directory under `_bmad/` or the two exempted `.claude/skills/` paths can ever trip `stillOnDisk` — 37 tracked product skill dirs against 106 rows. The guard's own docblock says "add a name to RETIRED only when its row is actually gone AND its files are gone", which describes a check the code performs for a minority of rows. Not a regression (the original test had the identical basis) and not a false pass in the direction that matters most — it under-detects manifest gaps, it does not invent them. Not fixed here because the honest fix is to enumerate the installed tree rather than the tracked one, which changes what the audit's evidence base IS, and that is a design decision.

- source_spec: `scripts/audit/skill-manifest-integrity.js` (Fast Lane — no spec file)
  summary: four smaller gaps in the promoted audit, recorded at the Round 3 cap rather than patched in a fourth pass.
  evidence: Round 3 Blind Hunter, each verified. (1) **CLOSED 2026-09-09 by sp-7-1 AC3** — `row/malformed` now tests `row.length !== header.length`, so a row LONGER than the header is reported; a Round 1 follow-up also stopped such a row cascading into a false `roster-decay` that advised declaring a retirement. ~~`row/malformed` tests `row.length < header.length` only, so a row LONGER than the header (an unescaped comma splitting a description) audits clean.~~ Items (2)-(4) below remain OPEN. (2) The audit is one-directional: only names in a hardcoded roster are cross-checked, so a tracked product skill with no manifest row is invisible — 10 such directories exist today, including the two `.claude/skills/` ones. (3) `tracked-skills/empty` is masked by the `header/missing-column` and `manifest/empty` early returns, so the Round 1 headline signal is suppressed in exactly the two cases where the audit bails early (still exit 2, so the harm is diagnostic, not a false pass). (4) `--project-root --help` returns usage and exit 0 without auditing, because `--help` is tested before value analysis. Not fixed: `code-review-convergence` permits no Round 4, and this change already had three rounds with a self-inflicted defect in each.

## Deferred from: code review (2026-09-05)

- source_spec: `scripts/audit/name-registry-integrity.js` (T124, Fast Lane — no spec file)
  summary: `readManifest` loses any CSV cell containing a newline inside quotes, so a multi-line cell splits into two rows and reports `row/arity`, masking every content check on that row.
  evidence: Round 1 Edge Case Hunter, verified by execution against `scripts/portability/manifest-csv.js:119` — `readManifest` does `content.split('\n')` BEFORE `parseCsvRow`, so the parser never sees a quoted newline as part of a field. Its own sibling `writeManifest` can emit exactly that, so the round trip is lossy. Genuinely pre-existing: the parser predates this change, and `skill-manifest.csv` has the same exposure. Not fixed here because the honest fix is a quote-aware character-stream parser in `manifest-csv.js`, which changes a module consumed by the portability exporter and the skill-manifest lint — a shared-parser change with its own blast radius, not a patch to this audit.

- source_spec: `scripts/audit/name-registry-integrity.js` (T124, Fast Lane — no spec file)
  summary: `--root` points the CSV read at another tree while `agent-registry.js` is still required from THIS package, so auditing a foreign project reports every row as drift.
  evidence: Round 1 Edge Case Hunter, verified. `main()` resolves the registry under `projectRoot` but `operationalAgents()` closes over the module-level `require('../update/lib/agent-registry')`. Not fixed because `--root` exists as a fixture affordance for tests and falsifiability runs, where sharing this package's agent registry is the intended behaviour; making it project-relative would require resolving a second tree's `scripts/` and is only meaningful if the audit is ever pointed at an installed project, which repo-side-only scoping says it is not. Revisit if the registry is ever ruled to ship.

## Deferred from: code review Round 2 (2026-09-06)

- source_spec: `scripts/audit/name-registry-integrity.js` (T124, Fast Lane — no spec file)
  summary: ADR-001 D5 states that "deliverable 3's CI check asserts declared tier against reality", and it does not. The script validates tier membership in an enum and nothing else.
  evidence: Round 2 Blind Hunter, verified. `checkShape` tests `VALID_OWNERSHIP_TIERS.includes(...)` (named `VALID_TIERS` when this row was filed; renamed by sp-7-2 to end a collision with the portability vocabulary); nothing compares `tier: convoke` against the row's asset living under `_bmad/bme/`, or `tier: bmad-upstream` against it NOT living there. Not fixed because the assertion is only falsifiable for a minority of rows — agent rows resolve to a directory, but a team row's tier has no on-disk correlate at all, and the two tiers that would matter most (`practice`, `client`) describe content this repository cannot see by ADR-001 D6. Closing it honestly means deciding what evidence a tier claim is checkable against, which is an operator ruling. **This is a signed ADR naming an assertion that does not exist — the "governance instrument that does not run" class P60 was opened for.**

- source_spec: `_bmad/bme/_config/name-registry.csv` (T124, Fast Lane — no spec file)
  summary: `unassigned` is a fifth tier value the signed ADR-001 does not authorise; D4 declares exactly four.
  evidence: Round 2 Blind Hunter, verified — the strings `unassigned` and `reserv` each appear zero times in `adr/meta-model/adr-001-provenance-and-vocabulary.md`. The script's comments cited D4 and D8 for both; those citations were corrected on 2026-09-06 to name the real source (the 2026-08-15 baseline memo for reserving) or to state plainly that the ADR is silent. The remaining gap is the artifact itself: five proposed teams carry `tier: unassigned`. Resolve by amending ADR-001 or by using a blank cell — an operator ruling, not a patch.

- source_spec: `_bmad/bme/_config/name-registry.csv` (T124, Fast Lane — no spec file)
  summary: `declared_in` carries two incompatible meanings, and only the agent-row meaning is validated.
  evidence: Round 2 Blind Hunter, verified against the live file. Team rows use it for the provenance of the declaration (`taxonomy`, `both`, `ecosystem-doc`, `session-2026-09-05`); agent rows use it for the module (`vortex`, `gyre`, `loom`). `checkAgentSources` enforces the module reading on agent rows via `drift/module-mismatch`; nothing validates team rows, so `declared_in: banana` on a team row audits clean. Not fixed because splitting the column is a schema change to a governance artifact one day old, and the right shape (two columns, or an enum per kind) is a design decision.
...
## Filed from: code review of dist-2-3a, Round 1 (2026-09-06)

- **The shipped-link finding with no ADR-002 class needs an owner before dist-2-3c.** *(RESOLVED 2026-09-07 — ADR-002 Amendment 3(1): `_bmad/bme/_config/` does NOT ship; the link becomes a validated self-referential absolute URL. Adopted by `dist-2-3c` as **AC2b** with task T3a. The registry fails ADR-002's own `project-context.md` required-reading test, and whether it ever ships to operators stays with the meta-model baseline.)* `_bmad/bme/_enhance/workflows/initiatives-backlog/templates/lifecycle-process-spec.md:133` links to `_bmad/bme/_config/name-registry.csv` as the **name authority** ("where the two disagree, the registry wins"). `files[]` ships `_bmad/_config/skill-manifest.csv` — one named file, not the `_bmad/bme/_config/` directory — so the target is in the repository and not in the package. It arrived 2026-09-05 in `154719e3`, after ADR-002 enumerated its three classes, and belongs to none of them. **Consequence:** 2.3a + 2.3b + 2.3c leave the gate red at this one finding, and 2.3c's job is to wire it in BLOCKING — doing so unchanged puts a red gate on `main`. **Needs an operator ruling:** ship `_bmad/bme/_config/`, or repoint the link. Named in prose in the `dist-2-2` and `dist-2-3a` records and in `assert-shipped-links.js`'s header (which names both files). `try-fresh-install.sh` does **not** name it — it makes only the generic point that not every finding belongs to one of ADR-002's three classes; an earlier version of this entry claimed otherwise. Until now it had no owning story and no backlog row, which Round 1 of dist-2-3a raised as epic residue.

## Filed from: code review of dist-2-3a, Round 3 (2026-09-06)

- **`shippedBmeModules` silently drops `files[]` entries containing `..`, and nothing owns it.** `shippedBmeModules(['scripts/../_bmad/bme/_ghost'])` returns no module and no `unresolvable` entry, while npm resolves the traversal and ships `_ghost` — verified against real npm with a throwaway fixture. So a module npm ships is neither counted in the expectation set nor reported, and `assert-installed-tree.js` exits 0 having never looked at it: the exact silent coverage-shrink the `unresolvable` channel exists to close. Backslash escapes (`\_bmad/bme/*`, `_bma\d/bme/*`) behave the same way. **Pre-existing** — found while reviewing a `dist-2-3a` change to this function that was subsequently reverted in full, so the defect is in the baseline and this story no longer touches the file. Not currently reachable: no `files[]` entry uses `..` or an escape. **The general lesson is the more useful output:** four hand-written attempts to reason about npm's glob semantics were each broken by adversarial review; the durable fix is to ask npm (`npm pack --dry-run --json`) rather than to re-derive its pattern matching, which is what `tests/lib/format-conversion-exclusion.test.js` now does for the exclusion.

## Filed from: code review of dist-2-3b (2026-09-07)

- **The Covenant ships to the package but never reaches an installed project — `dist-2-6`'s scope, second instance.** Measured with `KEEP=1 bash scripts/audit/try-fresh-install.sh`: `node_modules/convoke-agents/_bmad/bme/covenant/` holds both documents, `<project>/_bmad/bme/` holds only the five modules — no `covenant/`, no `README.md`. `grep -rn covenant scripts/update/` matches only `taxonomy-merger.js`; no install path copies it. This is the identical gap `_bmad/bme/_portability/` has and which the standing `[installed-tree status 1]` finding names. **It matters because the story's own premise is "the required reading my install tells me to read to be in my install"** — satisfied for the package, not for the project. Nothing in a project links to it today, so nothing breaks now. Note the interaction: `dist-2-3b` ships the Covenant as two NAMED FILE entries rather than a directory entry, which is correct (a `_bmad/bme/<name>/` entry is read as a module) but which also means the installed-tree gate will never report this gap — so it needs an owner rather than a check.
- **ADR-002 and ADR-005 carry stale reference links after the Covenant move.** The migration treats ADRs as verbatim on purpose: a first attempt rewrote ADR-002's evidence tables and ADR-005's normative prose, making signed records contradict their own evidence. But their *References* blocks are navigation, and `adr-005:87-88` now point at paths that no longer exist. Editing a signed decision record warrants an ADR amendment, not a silent patch — hence filed rather than fixed.
- **The only two governance documents that ship are no longer governance-checked, and both say `status: draft`.** Moving them to `_bmad/bme/covenant/` took them out of `artifact-utils.js`'s `includeDirs` scan scope (`planning-artifacts`, `vortex-artifacts`, `gyre-artifacts`), so `convoke-doctor`'s frontmatter checks no longer see `status`, `schema_version` or future drift on them. Per ADR-003 the class is a property of the artifact rather than the directory, so the frontmatter was kept — but nothing now validates it, and normative required reading ships marked `draft`.

## Deferred from: code review of dist-2-3c (2026-09-07)

Round 1 — three parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor), as
independent `claude-sonnet-5` subagents. 0 decision-needed, 3 patch (applied), 2 defer, 6 dismissed
(each re-verified rather than taken on faith — see the story's Review Findings section for the
verification each dismissal got).

- **`tests/lib/docs-packaging.test.js`'s `npm pack --dry-run --json` parsing has no shape or identity guard, and its reference-style-link regex misses indented definitions — both copied verbatim from `tests/lib/covenant-packaging.test.js`, which has the identical gaps.** Edge Case Hunter, Round 1. `JSON.parse(execFileSync(...))[0].files` assumes pure JSON on stdout and a single-entry array with no name check; an npm version that interleaves warnings, or a workspace-aware multi-entry output, fails opaquely instead of as a clear packaging-regression signal. Separately, `/^\[[^\]]+\]:\s*(\S+)/gm` anchors to column 0, so a reference-style link definition indented inside a list item or blockquote is never scanned. Both patterns are shared with `covenant-packaging.test.js` and, per Edge Case Hunter's own check, two files under `tests/unit/` — fixing only the newest file would leave three siblings carrying the identical gap. Fix path: a shared `packedPaths()` helper with the guard, and a shared link-scanning regex, both promoted out of per-file duplication into one home — a class-level fix, not a one-file patch.
- **`docs-packaging.test.js`'s "carries no relative link" test flags every relative link out of the migration guide, not just ones that would actually escape the shipped set.** Blind Hunter, Round 1. `relativeLinks()` has no notion of "resolves within `docs/migration/`", so a future same-directory cross-link (e.g. a second guide linking back to `3.x-to-4.0.md`) would false-fail — concretely reachable, since the suite's own first test explicitly anticipates a second guide file being added. Identical design already exists in `covenant-packaging.test.js:107-117`. Deferred rather than patched for the same file-vs-class reason as above, and because the direction is safe: over-strict fails toward a false failure, not toward the fail-open pattern this epic exists to close.
- **A symlinked `docs/migration/README.md` is silently omitted from the package regardless of `docs/.npmignore`'s anchoring.** Edge Case Hunter, delta review, verified against `npm pack --dry-run --json`. The Round 1 anchoring fix (`README.md` → `/README.md`) exists so a legitimate `docs/migration/README.md` CAN ship; that promise does not hold if the file is a symlink, because npm drops symlinked files, and nothing reports the omission. Zero symlinks ship today. Same class as the symlink gaps already filed from `dist-2-2` Round 3 against `shipped-links.js` (`..` traversing a symlinked directory; symlinked `.md` files dropped from the corpus) — worth closing as one group rather than three separate patches, since the durable fix in every case is to enumerate what npm actually packed rather than what the filesystem walk believes.
- **A case-variant stray README under `docs/` ships with its repository-only links.** Edge Case Hunter, delta review. `docs/.npmignore`'s `/README.md` is case-sensitive, so `docs/Readme.md` or `docs/README.MD` is not excluded and would arrive in the package carrying the same seven links into repository-only siblings that `docs/README.md` carries. Latent — no such file exists. Notable because this repository's own development filesystem (macOS, case-insensitive) cannot hold both spellings, so a contributor cannot reproduce it locally and it would surface only in CI, on a case-sensitive runner. Fix path is a case-insensitive pattern or a CI assertion that `docs/` contributes exactly the migration guide to the tarball — the latter is what `tests/lib/docs-packaging.test.js` already almost does.

## Deferred from: code review of dist-2-6 (2026-09-07)

Round 1 — three parallel layers as independent `claude-sonnet-5` subagents. 16 raw, 13 after dedup:
0 decision-needed, 8 patch (applied), 5 defer, 3 dismissed. No HIGH after triage.

**All five defers are one class: patterns the portability blocks MIRROR from the Artifacts blocks.**
Verified rather than assumed — the Artifacts loop at `refresh-installation.js:974` and its
`path.join(skillsDir, workflow.name)` at `:982` have the identical shape. `dist-2-6` copied the
Artifacts block deliberately, on the reasoning that making the module look like the one that already
works beats adding a sixth bespoke code path; the cost is that it inherited the gaps too. Fixing only
the portability half would leave the two halves of one contract disagreeing and close nothing as a
class, so they are filed together.

- **The standalone-workflow wrapper generator has no `Array.isArray` guard and no try/catch, while the validator that checks its output does.** `for (const workflow of <cfg>.workflows || [])` — a `workflows:` key written as a YAML mapping rather than a list is truthy, bypasses the `|| []` fallback, and crashes `for...of` with an uncaught TypeError mid-install. `validator.js`'s `validateStandaloneWorkflowModule` guards with `Array.isArray(config.workflows)` and wraps its loop in try/catch. So the writer and the checker of the same contract disagree about what a malformed config is, and the writer is the one that throws. Present in both the Artifacts block and the Portability block.
- **`workflow.name` is used verbatim in `path.join(skillsDir, workflow.name)` and in the source path, with no type or containment check — this is T102(a)'s defect on the WRITE side.** A name containing `..` or `/` is honoured by `path.join`, so a declared workflow named `../../../../tmp/x` writes its wrapper outside `.claude/skills/`; a non-string name throws. **T102(a) is the same hazard in the assertion that READS these wrappers** (a declared workflow named `../../../../tmp/x` is *satisfied* by a `SKILL.md` outside the project, reproduced at exit 0). One containment predicate should close both, and they should close together — a fix to only the reader still lets the writer create the file it would then be fooled by.
- **A config that parses to an empty or non-object value installs nothing and says nothing.** The "config.yaml not found" and "parse error" branches each `changes.push` a diagnostic the operator sees; the third bad-config branch does not, so the module is silently skipped. Both modules.
- **Wrapper-name collisions are silent last-write-wins.** Two workflows declaring the same `name` — within one config, or one in Artifacts and one in Portability, which now share the verbatim naming scheme — produce a single wrapper with no finding and no diagnostic. The `knownVerbatimNames` union that `dist-2-6` introduced makes the cross-module case newly reachable.
- **A source `SKILL.md` that is a directory passes every existence check.** `fs.existsSync` is true for a directory, and `fs.copy` would copy the tree into `.claude/skills/<name>/SKILL.md/`. Both modules.

## Deferred from: delta review of dist-2-6 remediation (2026-09-07)

Run because the set-equality check failed after Round 1's remediation — two source files had entered
the change set unreviewed, and a test had been rewritten. Not a Round 2 (no HIGH). It found that the
remediation had reproduced the defect it was fixing, which is recorded in the story; these three are
the items left open.

- **`WRAPPER_RULES.standaloneWorkflow` names the Artifacts generator as the representative site, so a portability wrapper failure points the operator at the wrong code path.** The rule is deliberately generic — it applies to every module declaring `standalone: true` workflows, which is what made it go red on `_portability` before dist-2-6 extended the generator, and that design was vindicated. But `assert-installed-tree.js` prints the rule's `site` verbatim in the failure message, and there are now TWO generator blocks (Artifacts and Portability, 6d and 6d-bis). An operator debugging a missing portability wrapper is sent to the Artifacts loop. The docblock is honest that the citation is a simplification; it does not state this cost. Fix needs a per-module `site`, which is a shape change to the rules table rather than a value correction — filed rather than patched inside a story that had already had one remediation pass reproduce its own defect.
- **`RUNTIME_DATA_FILES`'s `agent-manifest.csv` entry cites a declaration rather than the read, and has no `token` to make that detectable.** `readSite` lands on `const manifestPath = path.join(projectRoot, '_bmad/_config/agent-manifest.csv')`; the actual `fs.readFileSync(manifestPath, …)` is nine lines below. Because the entry sets no `token`, `auditCitations` falls back to matching the bare basename — which the declaration's own path literal satisfies — so deleting the read and leaving the declaration would keep the alarm green. This is the exact declaration-not-a-write defect that sibling entries in the same table call out in their comments and guard with a `token`. Pre-existing; the delta review corrected the entry's line number (`:286` -> `:289`) without noticing the entry needs a discriminating token to be worth citing at all.
- **The doc-comment preservation test is count-based, not content-based.** `tests/unit/refresh-installation-portability.test.js` counts comment lines in the source config and requires the same count after the version stamp. It would pass if the YAML round-trip preserved nine lines while scrambling their text. Verified by execution that the current `YAML.parseDocument` / `.set()` / `.toString({lineWidth:0})` round-trip is content-faithful (9/9 lines identical), so this is a narrower guarantee than the test's name asserts rather than a live failure. Fix is to compare the extracted comment text, not the count.

## Deferred from: code review of dist-2-7 (2026-09-07)

Round 1 — three independent `claude-sonnet-5` subagent layers. 13 raw, 8 after dedup: 0
decision-needed, 5 patch (applied), 2 defer, 1 dismissed. No HIGH. Acceptance Auditor found zero AC
violations and reproduced every Dev Agent Record claim exactly.

- **The REPLACEMENT side of `result.replace(re, replacement)` in `applyTransformations` is not escaped, and `sanitize.js` exports `escapeReplacement` precisely for it.** *(RESOLVED 2026-09-07 — **operator ruling: escape both sides**, the same answer FR16 got and for the same reasons. The precedent was already in this file, added by an earlier review finding: the template-directive rewrite calls `escapeReplacement` under a comment reading "Both sides need escaping." Ruling otherwise would have left one file with two rules for one construct, which is what the FR16 ruling refused. Enumerated first, and the enumeration was corrected at Round 1: **three** `.replace()` sites take a STRING replacement carrying external content — one already escaped (the template-directive rewrite), two not (these). Separately there are **three** replacer-function sites, which a Round 1 reviewer counted where the first draft of this note said two; they are immune by construction because a function never interprets `$`, but that framing flattered them — none routes `$`-bearing external data through the replacement anyway, so they were never in this risk class. Both string sites are now escaped. **No reachability pin was added, deliberately**: an unconditional escape has no safety argument to keep true, unlike the key pin, which exists precisely because the escaping there rests on a closure claim.

  **The declined alternative, stated correctly** — the first draft of this sentence did not parse. `sanitize.js`'s docblock recommends replacer functions over escaping where a caller can use one. Converting **only these two** sites would have left the third using `escapeReplacement`, which is the split the FR16 ruling refused; converting **all three** would have been consistent and is arguably the better end state, and was declined for scope, not for consistency — it touches a site this change otherwise has no reason to open. The earlier wording gave the two-site reason for the three-site action, which is incoherent, and was corrected here rather than silently dropped.)* Escaping the pattern is half the job: a replacement string containing `$&`, `` $` ``, `$'` or `$1` re-injects matched text into the output — `escapeReplacement`'s own docblock says so. Not reachable today: all six `CONFIG_VAR_MAP` values are hardcoded literals with no `$`, and `dist-2-7` did not change them. **This is FR16's symmetric question for the replacement side**, and it deserves the same ruling FR16 got rather than being decided silently: escape uniformly so a grep can confirm it, or exempt with the reachability pinned by a test. Note the pin `dist-2-7` added covers KEYS only — if the map ever takes a value that is not a hardcoded literal, both halves need re-checking. Raised at Round 1 because the implementer noticed it while writing the fix and did not act on it.
- **Obsolete on arrival, recorded for the lesson: a template-literal VALUE containing a newline followed by `  };` would have truncated the pin's source capture, hiding every key after it while `keys.length >= 6` still passed.** Real against the source-parsing pin that `dist-2-7` shipped mid-story; harmless against the one it ended with, which reads `Object.keys` of the exported object and has no capture to truncate. Kept because it is one more instance of the same conclusion the story reached the hard way — a source parser has to be taught every syntax the language allows, and the one it has not been taught yet is the one that bypasses it.
- **`CONFIG_VAR_MAP` is exported mutable and unfrozen, and so is its sibling `ALLOWED_WARNING_TYPES`.** Delta review of dist-2-7. Both sit in `export-engine.js`'s `// Internal helpers exported for testing` block; either could be mutated through the cached module object by a test or a downstream script and stay corrupted for the rest of the process. Not a live hazard today — Node's test runner isolates each `*.test.js` into its own process, and nothing in the repo mutates either — and **not novel to dist-2-7**, which merely added the second instance. `Object.freeze` on both costs nothing. Filed as one decision about the export block rather than a patch to one member, because freezing one of two identical siblings is the arbitrary inconsistency FR16's own ruling argues against.

## Deferred from: Round 1 of the replacement-side escaping (Fast Lane, 2026-09-07)

Fast Lane, no story file — Round 1 ran at the commit-preparation landing point. 5 findings: 3 patch
(applied), 2 defer. No HIGH. The fix itself was verified by execution: `escapeReplacement` is a
confirmed no-op on all six real `CONFIG_VAR_MAP` values, and the new source-shape test was
mutation-tested red.

- **The site that motivated the whole ruling has no source-shape pin, while the two it motivated now do.** The template-directive rewrite in `scripts/portability/export-engine.js` calls `escapeReplacement` under the comment *"Both sides need escaping"* — it is the precedent the 2026-09-07 ruling rests on — but nothing pins it. A regression there is caught only indirectly by functional tests, whereas the two `CONFIG_VAR_MAP` loops are now covered by an assertion that fails loudly on both a revert and a wrong-identifier swap. Pre-existing and out of scope for a change that did not open that site, but the asymmetry is backwards: the exemplar is the least protected of the three.
- **`export-engine.js` line 894 contains an identity replacer that is almost certainly a bug: `.replace(/^[a-z]/, (c) => c)`.** It matches the first lowercase character and substitutes it with itself — a no-op — immediately after a `.toLowerCase()` call, in a string being built for operator-facing prose. The surrounding intent reads as sentence-casing, and the sibling at line 1011 does exactly that with `(c) => c.toUpperCase()`. Found by a Round 1 reviewer inventorying every `.replace()` in the file for the escaping audit, not by anything looking for this. Pre-existing, unrelated to escaping, and left alone because a change to generated operator-facing text deserves its own verification rather than riding along in a security-hardening commit.

## Deferred from: code review of dist-2-8 (2026-09-07)

Round 1 — three independent `claude-sonnet-5` subagent layers. 16 raw, 11 after dedup: 0
decision-needed, 7 patch (applied), 4 defer. No HIGH. **The most consequential finding was a false
claim in the implementer's own record** — a "correction" of the story's module split that was itself
wrong, grouped on a different basis; see the story record.

- **Test 1b is FLAKY under the full parallel suite, and it is the only test in the repo that validates against the live `REPO_ROOT`.** Blind Hunter hit a false red once in four `npm test` runs: Test 1b reported `[BROKEN-DEP] bmad-advanced-elicitation` against an unmodified tree, confirmed clean by `git status`, `git diff` and a standalone re-run of the same `validate()` call returning zero findings. Three subsequent runs were clean. ~~`npm test` runs files concurrently and several suites call `process.chdir()` (`migration-runner-orchestration`, `migration-runner-rollback-bug8`, `registry-writer-dirty-tree`), which is process-wide and can race any concurrent `findProjectRoot()`.~~ **THAT MECHANISM IS FALSIFIED (2026-09-08).** `node --test` runs each FILE in its own child process: two probe files report different pids, and a `process.chdir()` in one leaves the other's cwd untouched (measured on Node 25.8.1, where the default is `process` isolation). **SCOPE LIMIT, stated rather than left implicit:** `engines` declares `>=18` and CI runs an 18/20/22 matrix, and only 25.8.1 was measured. `test-runner.js` already carries a comment recording that this codebase was burned once by assuming `node:test` behaves uniformly across 18/20 vs 22+, so "FALSIFIED" is established for the local version and *believed* — not measured — for the CI legs. No other suite's `chdir` can reach Test 1b's process, so the recorded diagnosis was wrong and a wrong recorded diagnosis is worse than an open one. **The mechanism remains unpinned.** What WAS fixed: `REPO_ROOT` was `findProjectRoot()`, which walks up from the working directory and returns any `_bmad`-bearing ancestor — `null` from `/tmp`, or a temp project root after a same-process `chdir` into one of the throwaway projects other suites build, each carrying its own seeded `skill-manifest.csv`. It is now derived from `__dirname` and guarded structurally (three branches, each proven RED when the guard is removed, plus a pin that the guard does NOT key on the package name — this package was renamed once already). The hazard is REAL but INERT under process-per-file isolation. Of the three `chdir`-calling suites, TWO — `migration-runner-orchestration` and `migration-runner-rollback-bug8` — build their temp projects via `createInstallation()` -> `refreshInstallation()`, which really does seed a `_bmad/_config/skill-manifest.csv` (`refresh-installation.js:587-684`); those are the loaded trap. The third, `registry-writer-dirty-tree`, does NOT: its `makeRepo()` builds a bare git repo holding one file, with no `_bmad/` at all, so a `chdir` there would make `findProjectRoot()` walk up and return `null` — a different failure shape, and a louder one. An earlier version of this entry said all three seeded a manifest; it was asserted, not checked, and review caught it. Nothing can currently spring either shape. **The observed red is not merely unpinned, it is structurally unexplained.** `bmad-advanced-elicitation`'s manifest row has an EMPTY dependencies column, and `checkRowDependencies()` returns before attaching any finding when `!depsStr` — findings are keyed `${type} ${skill}` on the offending row's OWN name, so `[BROKEN-DEP] bmad-advanced-elicitation` cannot be produced from this manifest by this code. Whatever was read that run, it was not the committed manifest. That is the thread to pull, and it is still open. Repeated clean full-suite runs were observed but are NOT evidence the flake is gone: the loop straddled the fix (started before it, edited mid-flight), so it is neither a clean baseline nor a clean verification — only a regression check that the change does not break the suite. **This predates `dist-2-8`** — Test 1b already read `REPO_ROOT` — but that story's gate table quotes a single clean run as though it were a property, and it is not. Fix path is isolation: snapshot the manifest into a temp root, or serialise the suites that `chdir`.
- **No finding type covers "a SEEDING row's bare-name dependency points at a row that does not seed".** Two exist today: `bmad-help` → `bmad-quick-dev` and `bmad-migrate-artifacts` → `bmad-create-epics-and-stories`, both `bmm` rows the installer never seeds by design. `[ORPHAN-DEP]` cannot cover it — it is a typo check against the manifest's vocabulary and both targets are correctly-spelled real rows — and `[BROKEN-DEP]` checks path-shaped deps. Found independently by both Blind and Edge. Pre-existing: the vocabulary was already derived from all rows before `dist-2-8` added the filter. Closing it needs a **new finding type**, which is why `dist-2-8` did not improvise one; benign today because `bmm` is out of Convoke's seeding scope on purpose and the operator's own BMAD install supplies it.
- **The non-vacuity floor `totalSkills >= 25` is not derived from a structural minimum.** Measured 31; 25 is "somewhat below that", roughly six rows of slack. A legitimate consolidation of several `bme` skills could trip it and force a floor bump alongside an unrelated refactor. A principled floor would assert per-module minima (`core >= N`, `bme >= M`) rather than a single total. Annoyance rather than defect, filed so the next person to hit it knows the number was never principled.
- **`validate()`'s `rowFilter` aborts the whole run if it throws, while the per-row loop below it degrades gracefully into a `[MISSING]` finding.** The filter stage has no try/catch and no `typeof` check on the option. A filter that throws on one row takes the entire validation down instead of surfacing that row as a finding — an asymmetry with the code directly beneath it.

## Deferred from: code review of sp-7-1 (2026-09-09), Round 3 cap

`code-review-convergence` allows no Round 4. Three rounds ran; every round's HIGH findings were
defects in the previous round's corrections, so the schema-doc check was DELETED rather than
patched a fourth time (AC4 struck — see the story). These are the residue.

- source_spec: `_bmad-output/implementation-artifacts/sp-7-1-fold-the-schema-lint-into-the-audit.md`
  summary: doc-conformance checking for `_bmad/_config/portability-schema.md` needs a spec of its own before anyone reimplements it.
  evidence: three rounds, three HIGHs, all in this one assertion — substring shadowing (`### Tier classification rules` satisfies `## Tier`), then a `TypeError` crash, then a fence parser broken in both directions (verified on the live doc: a nested-fence example yields a false pass; one stray unterminated ``` yields four false accusations from a gate wired into `ci.yml:184`). The questions that were never asked are what makes it re-attemptable: what counts as a section, whether a fenced example counts, whether the check applies to a source checkout / a packed tarball / a seeded operator project. Answer those first.

- source_spec: `scripts/update/lib/refresh-installation.js` (no spec yet)
  summary: nothing seeds `_bmad/_config/portability-schema.md` into an operator's project, so a seeded `skill-manifest.csv` carries a tier/intent vocabulary with no definition in the operator's tree.
  evidence: Round 3, proven by two independent agents running a real install — `npm i <tarball>` then the installer into a fresh project yields `_bmad/_config/{agent-manifest.csv, agents/, bmm-dependencies.csv, skill-manifest.csv, taxonomy.yaml}` and no schema doc; zero references to `portability-schema` anywhere in `scripts/`. Adding it to `files[]` puts it in `node_modules/` only, which is why that attempt did not close it. Genuinely pre-existing; surfaced because three rounds of wrong scoping claims forced the question.

- source_spec: `tests/lib/portability-classification.test.js` (no spec yet)
  summary: nine files under `tests/lib/` still call `findProjectRoot()` and read live tree state as data; the SCOPE comment names one of them.
  evidence: Round 3 Blind Hunter, enumerated: `portability-canonical-format.test.js:72`, `drift-snapshot.test.js:9`, `manifest.test.js:25`, `portfolio-engine.test.js:23`, `pf1-judge-prompt.test.js:9`, `taxonomy.test.js:18`, `inference.test.js:26`, `pf1-validation-battery.test.js:9`, `fresh-install-health.test.js:16`. sp-7-1 closed only the `skill-manifest.csv` instance, which its comment states accurately. Round 1 of this story overstated the scope ("clean"); Round 2 replaced it with an understatement of the same class. The honest position is on the record; the remaining eight are unscoped work.

- source_spec: `scripts/audit/skill-manifest-integrity.js` (no spec yet)
  summary: a manifest row truncated below the `name` column is indistinguishable from a deleted row, so it still trips `roster-decay` and advises declaring a retirement.
  evidence: Round 3 Edge Case Hunter, verified — a row truncated to one field is absent from `namedRows`, so the roster cannot know it was ever there. Unlike the too-LONG case (fixed in Round 2), this is a genuine information limit rather than a bug: the name is unrecoverable. Closing it would need the roster to reason about row ORDINALS rather than names. Filed rather than fixed because the fix changes what the roster's evidence base is.

- source_spec: `scripts/audit/skill-manifest-integrity.js` (no spec yet)
  summary: `finding()`'s severity-axis rationale is orphaned — the comment explaining why there is no severity axis sits above an unrelated constant block, and `const finding` carries none.
  evidence: raised in Round 1 (#14), marked applied in Round 2 while only the paired half (the PASS line) was actually fixed, and confirmed still open in Round 3. Cosmetic, but it is the third round it has survived, which is itself the point: a `[x]` was recorded against work that did not happen.

## Deferred from: code review of sp-7-2 (2026-09-09), Rounds 1-3

- source_spec: `scripts/audit/name-registry-integrity.js` (no spec yet)
  summary: `VALID_OWNERSHIP_TIERS` is unpinned — adding a sixth value reddens nothing.
  evidence: Round 1 Acceptance Auditor, verified by execution (75/75 still pass with an extra value). Out of sp-7-2's AC scope, which asked only for a rename to end the identifier collision. Noted because the renamed vocabulary now sits beside the portability set whose unenforced-agreement property this very story closed: the registry's tier column has one declaration, one consumer and no cross-check, so a widening there is invisible. Closing it needs a decision about what the registry's vocabulary is pinned AGAINST — there is no writer/checker pair here, so the sp-7-2 shape does not transfer.

- source_spec: `tests/audit/skill-manifest-integrity.test.js` (no spec yet)
  summary: the vocabulary pin interpolates a filename into `new RegExp` without escaping.
  evidence: Round 1 Edge Case Hunter, verified — inert for all three current basenames (the constructed pattern is byte-identical to the literal it replaced, and does not over-match superstring filenames), but a fourth copy named `vocab.shared.js` yields `.` as a wildcard and matches `require('./vocabXshared')`. **Fully moot as of Round 2** — the literal-declaration check that carried this regex was deleted, so no such code remains. Kept only as a note for whoever attempts the AST approach: `base.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')`.

- source_spec: `tests/audit/skill-manifest-integrity.test.js` (no spec yet)
  summary: nothing detects all three classification-vocabulary copies being widened TOGETHER through a shared module — the pin catches drift and shared instances only.
  evidence: sp-7-2 Rounds 1 and 2, four executed defeats across two attempts. A copying import (`[...require('./shared').TIERS]`) yields a genuinely independent array, so identity passes; all three drift together, so value agreement passes. Round 1's filename denylist and Round 2's array-literal source check were both defeated, the latter four ways including a `push()` after a pristine literal (`const` binds the reference, not the contents). Detecting this in source is not achievable by regex, and the literal check also false-accused legitimate values containing `.` or `require`. **This bears directly on the "give the vocabulary one home" item elsewhere in this file: that refactor is exactly what ships green while widening every gate.** Any future consolidation needs to answer this property by design — an AST check — a ~30-line `acorn` prototype was demonstrated during Round 3 to pass all three live copies and reject all four defeats without false-accusing legitimate values, and `acorn` already resolves in this tree via eslint — a generated single source with generated checkers, or an accepted documented risk — not by another source regex.

## Deferred from: code review of docs-1-1 (2026-09-11)

Round 1, three blind layers against commit `2c74deb6`. All five re-derived against the tree before deferral.

- **Three unresolvable `/bmad-bmb-*` commands in `docs/faq.md:131-135` and `docs/agents.md:357`.** `/bmad-bmb-agent`, `/bmad-bmb-module`, `/bmad-bmb-workflow` have zero rows in `_bmad/_config/skill-manifest.csv`; shipped ids are `bmad-agent-builder`, `bmad-module-builder`, `bmad-workflow-builder` (manifest 68, 71, 72). Same ACT-FAIL class as D1, different command family — 65 lines above the `faq.md:200` line docs-1-1 fixed. AC1's grep was the literal string `/bmad-team-factory`, so the class check never ran. `docs/agents.md` is recorded `Examined: yes` (Story 1.2, 4 findings), so `:357` is on the books as clean while carrying a command that does not exist — a coverage hole, not just a defect. Related: `docs/development.md:79` now routes readers to "BMB (Bond)" without naming a command, and the only doc that names one is wrong. Owner: Story 1.4 for `faq.md`. **`agents.md` closed by Story 1.2 (finding A6), 2026-09-11** — no re-opening needed.
- **D3 class live as copy-paste commands in `docs/BMAD-METHOD-COMPATIBILITY.md:139,261,263`.** Three lines reference `_bmad/bme/_vortex/agents/<name>.md` (flat). Vortex is `<dir>/SKILL.md`; `ls _bmad/bme/_vortex/agents/contextualization-expert.md` → No such file or directory. Lines 261-263 sit in a release checklist the reader is told to run. AC3 scoped its enumeration to `development.md`, which is defensible. Owner: Story 1.5.
- **`sprint-status.yaml`: 29 `abs-*` rows (v4.1 absorption epic, generated 2026-09-10 by sprint-planning) committed inside a docs fix, undisclosed.** 67 insertions / 1 deletion; the only deletion is `last_updated`. Nothing pre-existing was altered or dropped — the whole-file staging hazard docs-1-1 flagged did **not** materialise. The issue is disclosure: the File List says "(status transitions)" and the commit message is silent, so `git log` shows another initiative's scaffold landing in a docs commit. `commit-preparation` field 1. Already committed; no action beyond noting the hygiene gap.
- **`fast-readme-rewrite-lifecycle-spine.md:274` records D1 as open while the findings note records it closed.** Unchecked item: "`/bmad-team-factory` is not a registered skill; the id is `bmad-agent-bme-team-factory` [README.md:104]". Neither closed nor cross-referenced by docs-1-1.
- **Coverage table `Assertions` cell is `—` for both examined rows while Story 1.4 is the designated calibration point** (`convoke-note-docs-accuracy-findings-4-0-2.md:188-189`). docs-1-1 bumped `development.md`'s findings 7 → 9 and left the denominator empty, so findings-per-assertion has no baseline.

## Deferred from: Round 2 code review of docs-1-1 (2026-09-11)

Round 2 ran three independent blind layers on `06a452cd` — the Round 1 *remediation*. Its HIGH findings were predominantly defects in Round 1's corrections, which is `code-review-convergence`'s trigger to **change the instrument rather than patch again**. Everything below is therefore deferred by ruling, not by triage fatigue. Counts and anchors in the findings note are frozen pending Story 1.7 (see that note's banner).

**D2 class — live instances no grep can reach.** The class is NOT closed; only 5 locations across 4 files are.
- `CREDITS.md:36` — "Team Factory 🏭 — builds new BMAD-compliant teams, **adds agents to existing teams, adds skills to existing agents**". **This file ships** (`package.json` `files[]`) and is absent from every pattern's file list. Highest-consequence survivor: an npm user reading CREDITS is told the unbuilt capability exists.
- `docs/BMAD-METHOD-COMPATIBILITY.md:162` — `| 3.0.0 | … | Team Factory extensions, … |` contradicts `:105`, which Round 1 fixed. **No `add` token** — structurally unreachable by grep.
- `UPDATE-GUIDE.md:75` and `CHANGELOG.md:273` — "Extension validator — validation for factory-generated agent and skill extensions", offering a validator for artifacts that cannot be produced, two lines below their own refutation. No `add` token.
- `CHANGELOG.md:217`, `:271` — reverted at Round 2 by ruling; they carry the original claims again pending the CHANGELOG scope ruling below.
- Also shipped and outside every file list: `SECURITY.md`, `docs/migration/3.x-to-4.0.md`.

**Operator ruling owed — does `CHANGELOG.md` enter a present-tense derivation pass?** The repo already codifies the opposite: `scripts/docs-audit.js:541-542` skips CHANGELOG's stale-reference and broken-path checks because *"historical entries reference files that may have been deleted or renamed in past versions"*, and the findings note exempts dated historical records on the same reasoning. Round 1 rewrote two shipped-release entries anyway and introduced a false absolute doing it (see the story record). Both reverted. This needs a ruling, not a story-authored epic amendment.

**Story 1.6's ACs do not cover its enlarged scope.** The epic now admits `README.md` (owner: 1.6), but `convoke-epic-docs-accuracy-4-0-2.md:452` still enumerates five files and `:466` still reads "**all five files** carry `Examined: yes`". Story 1.6 cannot satisfy its own closing AC over a 13-file scope. Story 1.7's derived in-scope set is likewise framed around documents "under `docs/`" and has no statement covering root-level files.

**`[AR]` does not resolve for any installed operator, and the menu handler may not accept it anyway.** Round 1 repointed `_bmad/bme/_team-factory/agents/team-factory.md:123` from `architecture-reference-teams.md` to `loom-arch-reference-teams.md`. The new target exists — but only in a dev checkout: `_bmad-output/` is not in `package.json` `files[]` and no installer writes it. Separately, the handler contract at `:71` declares `data="path/to/file.json|yaml|yml|csv|xml"` — `.md` is not in it, so whether `[AR]` renders anything is unestablished. The filename fix is kept as strictly better than a name that resolves nowhere; the route is still broken. Per `feedback_verification_basis`, file existence is not the basis the claim is about.

**The `architecture-reference-teams` rename is half-applied.** 17 markdown files still name the dead artifact, including the live architecture document `_bmad-output/planning-artifacts/loom-arch.md:655,820` and both `*-note-validation-report.md:12`.

**`_bmad/bme/_team-factory/agents/team-factory.md:3` overclaims at every activation.** `description: "Team Factory - Guided creation of BMAD-compliant teams, agents, and skills"` contradicts the module's own `config.yaml:2`. The wrapper loads the full agent file at activation, so this is in-context every run. No `add` token.

**`grep -vE` line-drop exclusions suppress true positives.** The Round 1 pattern's `grep -viE 'add(ress|itional)'` drops the whole **line**, so "add an **additional** agent" is silently discarded. Filter the token, never the line.

**Commit message hygiene.** `06a452cd`'s body opens `ound 1 review of 2c74deb6` — a truncated "Round". The same malformed-message class the story's Dev Notes flagged on `e1510517` with "do not copy the pattern". Also: `06a452cd` staged 12 files against a 7-file File List that explicitly said **"Not mine, do not stage"** for `convoke-note-initiative-lifecycle-backlog.md`. Nothing was damaged — row count 139 → 139, prior text preserved verbatim, `backlog-integrity.js` exit 0 — but a Loom governance ruling now sits inside a docs commit, and this is the *second* consecutive docs-1-1 commit to sweep in foreign work.

## Deferred from: code review of docs-1-5 (2026-09-13)

- **`checkDocsCoverage` is aggregate, not per-file.** Admitting any document to `USER_FACING_DOCS` can only satisfy more agent/workflow coverage requirements, so an agent that disappears from every genuinely user-facing doc can be kept green by a single mention elsewhere. The mechanism is real. **Measured 2026-09-13: exposure from this story is ZERO** — the playbook mentions **0** agent names, and coverage findings are 0 both with and without it in the corpus, so it masks nothing today. Re-measure before acting: diff `checkDocsCoverage(all)` against `checkDocsCoverage(all minus <doc>)`. (`scripts/docs-audit.js:401`)
- ~~**`_bmad-output/` paths are unguarded by `checkBrokenPaths`.**~~ **CLOSED 2026-09-13.** The alternation now lists `_bmad-output` *before* `_bmad` — order matters, because alternation is first-match-wins and `_bmad` matched the prefix then failed on the required `/`, silently dropping the whole path. Measured before changing anything: across the corpus it newly checks **6** paths and the only two that do not resolve are in `CHANGELOG.md`, which `runAudit` exempts — so **0 net new findings** and the gate stayed green. Two tests added; the guard was shown live by breaking a real citation in memory (0 → 1 finding).
- ~~**The shipped package audits 11 of the 17 docs it declares.**~~ **LARGELY MOOT — established 2026-09-13.** Two things defuse it. (1) **`docs-audit.js` is not a bin**: `package.json` `bin` exposes 14 commands and none of them runs it (`node -e "console.log(Object.keys(require('./package.json').bin))"`), so no operator has a supported way to invoke it — it ships only because `files[]` contains `scripts/` wholesale. (2) The silent skip was **fixed in `2a95b23b`**: `runAudit` now reports every declared doc it could not read, so even a manual `node node_modules/convoke-agents/scripts/docs-audit.js` states its own reduced scope. What remains is a product question, not a defect: should `docs/` ship at all? No action unless the answer changes.
