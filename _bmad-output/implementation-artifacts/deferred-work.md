...
- **A39-D1 (AA-H1) — AC5 ≤2-sentence cap exceeded across ~6 cells** — Acceptance Auditor finding. Spec AC5: "≤ 2-sentence evidence note." Several §4 cells exceed: Scout R5 (~4-5 sentences before R1 condensation patch), Scout R7 (~6 sentences charitable + strict + headline reasoning), Atlas R7 (~5 sentences workflow.md inheritance + sub-field counting + error-path concepts + headline), Coach R1 (~4 sentences file ref + R1 framing + Compare-with + secondary R1 weakness + load-bearing FAIL), Scout R1 (~3 sentences). Sweep across all 6 cells would touch ~150 lines of evidence-note prose; high risk of introducing semantic drift while compressing. P16 condensed Scout R5 inline (deferred Compliance Checklist OC-R5 tension to §4.5 Notes per AA-H1 condensation pattern), but the other 5 cells remain over-cap. Fix path: v4+ refresh applies the same condensation pattern (move secondary detail to §4.5 Notes + §9 ambiguity intake), OR a methodology amendment via A41 allows compound-mechanism cells to exceed 2 sentences with an explicit §Notes pointer convention. Defer reason: scope creep risk; A39 R1 patches are content-only; bulk evidence-note rewrite across 5 more cells crosses the structural-change threshold per code-review-convergence rule.
