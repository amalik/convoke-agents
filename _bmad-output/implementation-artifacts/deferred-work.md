...
- **R1-M4 — Template placeholders leak through to ALL 44 exported instructions.md** (Edge Case Hunter VERIFIED 44/44 affected: `[your name]`, `[your context]`, `[your preferred language]`, `[your output folder]`). BMAD-internal config-substitution markers should be either resolved with sane defaults at export time OR added to `FORBIDDEN_STRINGS` so `validate-exports` flags them. **Critically reveals Story 3.5 spot-check rubric is too narrow** — `[your name]` isn't in `FORBIDDEN_STRINGS` so the 9-cell rubric does NOT catch this class of leak. Two follow-up items: (a) Bug-Lane item against `scripts/portability/export-engine.js` to substitute defaults or extend FORBIDDEN_STRINGS; (b) Initiative-Lane item to widen export-rubric (post-EXP3-baseline reflection). RICE candidate: high reach (44/44 skills × 3 platforms = 132 affected adapters), medium impact (UX-visible to catalog users), high confidence (verified empirically), low effort (1-line fix to test-constants.js OR ~10-line fix to export-engine).
