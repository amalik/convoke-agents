...
- **`checkDocsCoverage` is aggregate, not per-file.** Admitting any document to `USER_FACING_DOCS` can only satisfy more agent/workflow coverage requirements, so an agent that disappears from every genuinely user-facing doc can be kept green by a single mention elsewhere. Green today; the mechanism is real and unexamined. (`scripts/docs-audit.js:401`)
...
- **The shipped package audits 11 of the 17 docs it declares.** Six `docs/*.md` entries — including the newly-admitted playbook — are not in `package.json` `files[]`. `runAudit` `continue`s past missing entries silently and exits 0, so an operator running `docs:audit` audits two-thirds of the declared corpus with no warning. (`scripts/docs-audit.js:716`)
...
- **`checkBrokenPaths`/`checkBrokenLinks` are not fence-aware.** An illustrative path or link inside a fenced example is treated as a real reference. Pre-existing across 15 other audited docs, but newly reachable from a playbook whose whole subject is the *next* release's not-yet-existing artifacts.
