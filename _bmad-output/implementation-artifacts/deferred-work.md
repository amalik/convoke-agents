...
- source_spec: `tests/audit/skill-manifest-integrity.test.js` (Fast Lane — no spec file)
  summary: the fixture `HEADER` is a 9-column literal with no linkage to the real manifest, so a column rename is not reproducible from the fixture suite.
...
- source_spec: `tests/lib/portability-schema.test.js` (Fast Lane — no spec file)
  summary: the `test-fixture-isolation` violation is NOT resolved as a class — a sibling test in the same directory still reads the live tree, and the classification vocabulary now exists in four independent copies.
