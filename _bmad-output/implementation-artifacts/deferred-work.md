...
- **`_bmad-output/` paths are unguarded by `checkBrokenPaths`.** Its regex alternation is `(scripts|docs|tests|\.github|_bmad)\/` — `_bmad-output/` fails it because the next character is `-`, not `/`. Nine of the playbook's fourteen links point into `_bmad-output/` and none are checked. (`scripts/docs-audit.js:363`, `T160` class)
