...
**Namespace decision:** no skill, agent or workflow is added. Changes are prose in shipped files, one changelog entry, and one checklist step — plus, from Round 2, `scripts/audit/check-changelog-entry.js` and its tests, a contributor-only release gate that lives beside the other `scripts/audit/` checks and is wired into nothing. `namespace-decision-for-new-skills` is satisfied because no skill, agent or workflow namespace is touched.
...
| Date | Note |
|---|---|
| 2026-09-17 | **Closed.** Story-close consumer audit found one defect and corrected it: `customize-without-forking.md` carried a sentence this story had garbled in Round 1 (a dangling "rather than replacing it with defaults") which also understated `T181`. The story header's "changes are prose" was true when written and false by Round 2; corrected. Everything else the audit reached — README, `project-context.md`, the publishing playbook, the seven guides, the nine activation blocks — carries no stale claim about the config refusal. |
