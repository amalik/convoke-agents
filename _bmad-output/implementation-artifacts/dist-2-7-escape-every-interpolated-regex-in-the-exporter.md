...
| Date | Change |
|---|---|
| 2026-09-07 | **Closed. CI green on `3482215b`** (`agent-surface-parity` and `fresh-install` both success). All six ACs met. **FR16 now holds literally** — all five interpolated `RegExp` constructions in `export-engine.js` escape their value, so the rule is one a grep can confirm and no exemption list exists to rot. Shipped across two commits: `86bf9ecb` (the ruling and the story) and `3482215b` (the fix). Two review passes; **3 items deferred**, the sharpest being that the REPLACEMENT side of `result.replace` is unescaped — FR16's symmetric question, which deserves the same explicit ruling rather than a silent judgement. |
