...
patterns widened again: it reports a **FLOOR**, says so in its own output, and the known-missing classes (no total here — `T160` has since closed one and added two, so the
findings note is the count) are tabulated in the findings note and filed as `T160`. A floor still sizes work ordinally; a
