...
**Principles:** Max has **7**, so there is slack against `countRules >= 5` — but render them as bullet lines matching `/^\s*[-*]\s/` regardless. The 7, verbatim from the v5 `<principles>`:
