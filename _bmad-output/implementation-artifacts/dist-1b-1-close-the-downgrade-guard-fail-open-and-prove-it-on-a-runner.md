...
| Date | Change |
|---|---|
| 2026-08-23 | **Code review R1 — 3 layers on the shipped guard. Several HIGH, all in code I wrote.** **The `sort -V` argument was false.** Both shape checks used `^[0-9]+\.[0-9]+\.[0-9]+$`, which ACCEPTS `4.01.0`/`04.0.0`/`4.1.00`; the regex that rejects leading zeros, `SEMVER_RE`, constrains TAG/VERSION only -- it never sees `CURRENT` (which arrives from the registry) and the dry caller has no `SEMVER_RE` upstream at all. So the comment described a property the code lacked, and its real load-bearing clause was *"the registry cannot hold a non-semver latest"* -- **an assumption about npm, made inside a block that refuses to trust the registry on line count, prerelease shape and build metadata**. Both regexes tightened; the claim is now true of the code. **A genuine FAIL-OPEN on the E404 anchor:** a well-formed `npm error code E404` proves *npm said 404*, not *this package is unpublished* -- a misdirected registry 404s for a PUBLISHED package and the guard would skip. T46's own filed remedy (a second probe) was **tested and does not discriminate** -- both cases 404; the resolved REGISTRY does, so the fix pins `npm config get registry` and requires the 404 URL to be exactly `<registry><package>`. **AC5's zero-padded matrix cases were missing** -- the only inputs with information content about GNU-vs-BSD -- and the dry job **had no `if:`**, so it fired on tag pushes, contradicting Task 4's own recorded decision. **Nothing asserted the publish job still CALLS the script**: the extraction created a seam where deleting the call leaves the dry job green forever. Added `tests/unit/publish-guard-wiring.test.js`, proven by mutation |
| 2026-08-23 | **Code review R2 — 5 more findings, every one in R1's fixes.** **Two of the three zero-padded cases passed for the WRONG reason:** `run_case` compared only OK-vs-FATAL, so with the regexes reverted to their loose form `4.01.0/4.1.0` and `4.0.1/4.1.00` still FATAL via the DOWNGRADE branch and sailed through the regression they exist to catch -- only one of three was load-bearing. Worse, `4.01.0` vs `4.1.0` is by the file's own argument a numeric TIE, so its expected value under the mutant depends on GNU's tie-break: **a test whose result depends on the implementation it is meant to be independent of**. `run_case` now takes an optional reason and all three fire against the mutant. **Scoped packages could never match** -- npm URL-encodes `@scope/name` as `@scope%2fname`, so a first scoped publish would FATAL with the wrong cause on a spent tag. **The new test guarded the call site but not its position or reachability:** three mutations that fully neutralise the guard (publish moved above the block, block made unreachable, DIST_TAG derivation inverted) left all five assertions green; three more added, each proven by mutation. **And R1 tightened the script while leaving two copies of the same claim wrong in `ci.yml`** -- its own CAND check still loose, its BSD/GNU comment still crediting `SEMVER_RE`. **Verified CLEAN by the reviewer:** leading-zero exclusion IS sufficient for BSD/GNU agreement, and all five original assertions were load-bearing. **Recurring bug in my own checks, now fixed structurally:** three times in this file I anchored on `indexOf('downgrade-guard.sh')`, which matches a COMMENT; extracted a `guardCallLine()` helper so it cannot recur. Suite 1662/1663, 0 fail; matrix 10/10 with reasons |
