...
| Date | Change |
|---|---|
| 2026-09-07 | **Closed. CI green on `8e95d8fd`** (`agent-surface-parity` and `fresh-install` both success). Shipped across two commits: `c4a3c18c` (the module work) and `8e95d8fd` (the install-scope snapshot acknowledgement). **`_bmad/bme/_portability/` is reachable** — four skills that shipped to every operator and reached none of them now install and invoke, proven on a fresh temp install. The installed-tree assertion went from one standing finding to `[installed-tree status 0]`. Status → `done` on **seven of eight ACs**: AC8 (wire `$TREE` blocking) was deferred by operator ruling and is carried by **T102**, whose row records that both preconditions are already met, so clearing (a)-(f) leaves only a one-line verdict edit. Two review passes plus a post-commit CI fix; **8 items deferred**, and **T111 sharpened** with the six-writes-counted-as-one instance this story produced. |
