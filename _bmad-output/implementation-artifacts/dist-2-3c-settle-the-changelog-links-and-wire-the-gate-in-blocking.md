...
| Date | Change |
|---|---|
| 2026-09-07 | **Closed. CI green on `aad6bd29`** (CI + CodeQL success; `fresh-install` and `agent-surface-parity` both green). Shipped across two commits: `018ec89e` (the gate and the fixes that make it green) and `aad6bd29` (the T32 correction). **FR12 is enforced** — `fresh-install` went green on `018ec89e` with `$LINKS` in the blocking verdict, the first CI run in which a broken shipped link could fail the build. Status → `done`. Three deviations stand in the record rather than reframed as compliance: AC1 created a finding class the story did not anticipate; AC6 asked for a note that is false (FR13 is NOT enforced — `$TREE` stays outside the verdict until `dist-2-6`); and this story does not close T32, which was already closed by `4556f4f0`. Two review passes plus a post-commit correction; 6 items deferred to `deferred-work.md`. |
