...
| ~~**D47**~~ | `what-convoke-brings:18` | *(withdrawn)* | **RETRACTED at Round 2 — the finding was false and its remedy deleted an owned, true claim.** `agent-manifest.csv` (tracked, generated by `scripts/generate-manifest.js`) lists `dev/Amelia`, `qa/Quinn` and `sm/Bob` identically; Quinn's and Bob's skills are **registered** under BMM's implementation phase (`bmad-qa-generate-e2e-tests`, `bmad-sprint-planning`, `bmad-create-story`, `bmad-retrospective`) — ⚠ registered, **not shipped**: no `_bmad/bmm/` file is in the tarball, which is `D46`'s own finding, and Amelia's paths are absent identically. The phase registration is what the restored sentence describes; and `CREDITS.md:53-54` asserts both — **edited in the same commit that deleted them from the sibling file.** The instrument was the defect: `grep -ci <persona>` searches a *description* column, and Amelia survived only because `bmad-agent-dev`'s description says "talk to Amelia". Sentence restored | `grep -iE '^"(dev\|qa\|sm)"' _bmad/_config/agent-manifest.csv` → all three · `grep -n "Quinn 🧪 QA" CREDITS.md` |
...
| **D49** | `README.md:98` | "what travels cleanly today is the upstream BMAD skill set plus that one" | **False on the file's own definition — and the obvious way to count it is wrong too.** BMAD proper is **72 rows: 43 `standalone`, 6 `light-deps`, 23 `pipeline`**, and `pipeline` is what the same line calls "flagged non-portable". Counting every non-`bme` row instead gives 87 and 38, because all 15 `_bmad/wds/` rows are `pipeline` and `wds` belongs to neither project. A `pipeline` skill is not *refused* — it exports with a framework-only banner (`export-engine.js:1163`) | Parse `_bmad/_config/skill-manifest.csv` with a quote-aware reader (the description field contains commas, so `cut -d,` and `split(',')` both return noise) and group by the `module` and `tier` columns |
...
| **D51** | `docs/testing.md:17-25`, `:31-35` | per-suite `Tests` columns | **9 of 14 wrong**; 5 correct, so a spot-check passes. The tables also showed 9 of 49 unit files and 5 of 12 integration files while reading as a census. **Counts column deleted; both tables labelled a selection** | `git show 9e986db8:docs/testing.md \| sed -n '17,25p;31,35p'` names the 14 suites (the fix deleted the table, so they are recoverable only from the pre-edit file); then run each under its own directory — nine live in `tests/unit/`, five (`fresh-install`, `upgrade`, `cli-entry-points`, `installer-e2e`, `convoke-doctor`) in `tests/integration/`: `node --test tests/{unit,integration}/<name>.test.js \| grep '^ℹ tests'` |
...
| **D57** | `docs/testing.md:88` | Wade's Domain 4 "Error Handling" | The cited source says **"Output Quality"**. Domains 1-3 matched, so everything read verified unless you opened the target. ⚠ **And the headline above the table was fabricated**: this same source reports `**Executed:** 0 / **Passed:** 0 / **Pass Rate:** 0%` in its own header, against `docs/testing.md`'s claim of `18/18 PASSED`. Closed by **R2**'s deletion | `grep -n "^## Domain" _bmad-output/_archive/phase-2/wade-p0-test-execution.md` |
| **D58** | `docs/testing.md:90` | "Live Test Suite: 5/5 PASSED (activation, full workflow, validation, chat, party mode)" | **Only "activation" was real**; the other four named no test, and the 5/5 came from a file `testing.md` never cited. *(That file reports 5 executed and 5 passed — the `0%` header belongs to `D57`'s source, not this one.)* Closed by **R2**'s deletion | `grep -n "^### Test" _bmad-output/_archive/phase-2/wade-live-test-results.md` |
...
**All ten of AC2's classes were swept.** The findings each produced are the `D43`-`D62` rows above;
classes that produced none are recorded there by their absence, not by a tally here. *(A per-class
count was written here twice and was wrong both times — once by omitting behavioural prose, once by
conflating counted assertions with findings. The rows are the record.)*

**The counted assertions and the findings are DISJOINT sets.** `CREDITS.md`'s single counted assertion
is a path (`README.md`); `CODE_OF_CONDUCT.md`'s is the Contributor Covenant version marker. Neither is
`D43` or `D44`, and `D48`'s three `{target}/…` paths are residual candidates rather than counted ones.
Any subtraction of "findings" from "assertions" is therefore meaningless — do not compute one.

**Basis for the `Assertions` column: the files as received at `9e986db8`.** Re-derive with
`git worktree add --detach <dir> 9e986db8`, then the AC1 command. **No post-pass figure is recorded
here** — it changes with every edit to these files, including the edits made while fixing them, and it
was written down wrong twice for exactly that reason. The script reports FLOORS and a non-zero residual
is a known gap (`T160`); run it rather than reading a number.
