...
| `docs/what-convoke-brings-to-bmad-method.md` | yes | 12 | **yes** | 1.6 | 2 |
...
**19 live findings across `D43`-`D62`** — 20 were filed and `D47` was **retracted at Round 2** (see its row). Seven files, **112** derived assertions as received at `9e986db8`. `docs/references.md`
...
| **D45** | `SECURITY.md:5` vs `:9` | "Only the latest published release receives security fixes" against a table row `4.0.x ✅` | **Self-contradiction**: `4.0.0` is published, undeprecated, matches `4.0.x`, and is not latest. Fixed the **prose**, kept `4.0.x`. ⚠ `4.0.2` was deliberately **not** written in — `package.json` says 4.0.2 but npm `latest` is `4.0.1`, so asserting it would name a version nobody can install | `npm view convoke-agents dist-tags` → `{ latest: '4.0.1', rc: '4.0.1-rc.0' }` · `npm view convoke-agents@4.0.0 version` → `4.0.0` |
...
| ~~**D47**~~ | `what-convoke-brings:18` | *(withdrawn)* | **RETRACTED at Round 2 — the finding was false and its remedy deleted an owned, true claim.** `agent-manifest.csv` (tracked, generated by `scripts/generate-manifest.js`) lists `dev/Amelia`, `qa/Quinn` and `sm/Bob` identically; Quinn's and Bob's skills ship (`bmad-qa-generate-e2e-tests`, `bmad-sprint-planning`, `bmad-sprint-status`, `bmad-create-story`, `bmad-retrospective`); and `CREDITS.md:53-54` asserts both — **edited in the same commit that deleted them from the sibling file.** The instrument was the defect: `grep -ci <persona>` searches a *description* column, and Amelia survived only because `bmad-agent-dev`'s description says "talk to Amelia". Sentence restored | `grep -iE '^"(dev\|qa\|sm)"' _bmad/_config/agent-manifest.csv` → all three · `grep -n "Quinn 🧪 QA" CREDITS.md` |
...
| **D49** | `README.md:98` | "what travels cleanly today is the upstream BMAD skill set plus that one" | **False on the file's own definition.** Of 87 non-Convoke rows in the shipped manifest, **43 `standalone`, 6 `light-deps`, 38 `pipeline`** — and `pipeline` is what the same line calls "flagged non-portable". ⚠ A naive `cut -d,`/`split(',')` returns noise or zero rows; the description field contains commas | `grep -oE '"[a-z]+","_bmad/' _bmad/_config/skill-manifest.csv \\| cut -d'"' -f2 \\| sort -u` lists the modules; for the tier split use a quote-aware CSV parse (a bare `cut -d,`/`split(',')` breaks on commas inside the description field). ⚠ **Count against BMAD proper, not all non-`bme` rows**: the 15 `wds` rows are all `pipeline` and `wds` is a parallel extension, so including them overstates `pipeline` from 23 to 38 |
| **D50** | `docs/testing.md:11` | "184 tests (130 unit + 54 integration) \| 83.4% line coverage" | The real total is roughly **17×** the stated figure (the story's AC2 preamble puts it exactly: off by 2,933), and coverage is higher. **The arithmetic was deleted, not corrected** (FR3a): nothing pins a test total and it rots on the next commit. Replaced by `npm run test:all` and `npm run test:coverage` | `npm test` · `npm run test:integration` · `npm run test:p0` · `npm run test:coverage` |
| **D51** | `docs/testing.md:17-25`, `:31-35` | per-suite `Tests` columns | **9 of 14 wrong**; 5 correct, so a spot-check passes. The tables also showed 9 of 49 unit files and 5 of 12 integration files while reading as a census. **Counts column deleted; both tables labelled a selection** | `git show 9e986db8:docs/testing.md \| sed -n '17,25p;31,35p'` names the 14 suites (the fix deleted the table, so they are recoverable only from the pre-edit file); then `node --test tests/unit/<name>.test.js \| grep '^ℹ tests'` per suite |
| **D52** | `docs/testing.md:11` vs `:31-35` | headline said 54 integration tests | **The table summed to 53.** Internal arithmetic — no source-derivation pass finds this; closed with `D50`'s deletion | `git show 9e986db8:docs/testing.md \| sed -n '31,35p' \| awk -F'\|' '{s+=$3} END {print s}'` → `53`. ⚠ **The `git show` prefix is required** — without it the command reads the file this commit rewrote and returns `0`, which reads as "claim disproven" |
| **D53** | `docs/testing.md:51` | "Six jobs run on every push and pull request" | **11 jobs; 3 conditional; 8 unconditional.** Five were omitted, and one of the six named (`publish`) runs on neither trigger. CI also runs on pushes to `main` and PRs to `main`, not every push | `node -e "const y=require('js-yaml'),w=y.load(require('fs').readFileSync('.github/workflows/ci.yml','utf8'));const j=Object.keys(w.jobs);console.log(j.length, j.filter(n=>w.jobs[n].if).length+' conditional')"` → `11 3 conditional`. ⚠ A bare `grep` on indented keys also matches `push:`/`pull_request:`/`run:` |
...
| **D56** | `docs/testing.md:40` | "`npm test` — Unit tests" | Runs **four** directories, three of which are not `tests/unit`. Four scripts were absent from the block — `test:p0`, `docs:audit`, `check` and `refs:audit`; all four are now listed | `node -e "console.log(require('./package.json').scripts.test)"` |
...
| **D58** | `docs/testing.md:90` | "Live Test Suite: 5/5 PASSED (activation, full workflow, validation, chat, party mode)" | **Only "activation" was real**; the other four named no test, and the 5/5 came from a file `testing.md` never cited. ⚠ **Worse than recorded at first pass:** the cited source's own header reads `**Executed:** 0 / **Passed:** 0 / **Pass Rate:** 0%`, so the deleted table's `18/18 PASSED` headline was fabricated outright — the first pass checked the Domain-4 *label* and wrote "the counts were right". Closed by **R2**'s deletion | `grep -n "^### Test" _bmad-output/_archive/phase-2/wade-live-test-results.md` |
...
**Per-class sweep — all TEN of AC2's classes, `0` written as `0`.** *(Round 2 found the first pass
reported nine: **behavioural prose** was omitted, which is the class that produced `D46`, `D49` and
`D53`.)* statistics/percentages **6** (all six wrong — `D50`, `D54`, `D55`); **behavioural prose 3**
(`D46` "installs both", `D49` "travels cleanly", `D53` "runs on every push and pull request"; the
claims that HOLD include "works standalone", "no npm dependency on BMAD", and the Node 18/20/22
matrix); dates **2 in scope** — `references.md:6` HOLDS; ⚠ `what-convoke-brings:8` reads "April 2026"
and **this commit changed the file**, so the "matches its last commit month" basis no longer applies.
Read it as an authored date, which the git history still supports (`7cba96f5`, `2d77f875`, both
2026-04-24); unbackticked prose paths **0** (six real ones, all resolve — `Node.js` is a regex false
positive); requirement identifiers **0** across all six prose files; agent and team names **0** (all 7
Vortex and 4 Gyre names correct); CI job names **1** (`D53`); word-form counts **1** (`Six jobs`;
*"Twenty-two workflows"*, *"Five additions"*, *"Three capabilities"*, *"seven-stream"* all verified and
HOLD); table-internal arithmetic **1** (`D52`); partial-truth sentences **1** (`D60`).

**The counted assertions, dispositioned — not conflated with the findings.** The two are **disjoint
sets**, which the first pass's `1 | 1` rows obscured. `CREDITS.md`'s single counted assertion is a
**path** (`README.md`, `:79`) — it resolves. `CODE_OF_CONDUCT.md`'s is a **version** (`2.0`) — the body
is canonical Contributor Covenant 2.0 and provably not 2.1 (`grep -c caste` → `0`, a 2.1 addition).
Neither is `D43` or `D44`. Likewise `D48`'s three `{target}/…` paths are **residual candidates, not
counted assertions** — AC7 says so — so "112 minus 20 findings" was never the right subtraction.

**Basis for the `Assertions` column: the files AS RECEIVED at `9e986db8`, not at HEAD.** Re-running the
AC1 command after the pass returns **149** (and residual 119), because a derivation pass adds the
reproducing commands NFR1 requires. Both readings are correct for different questions; the column
records the story's *load*, which is what Story 1.4's projection of 112 sized and what Story 1.7's gate
consumes. Reproduce the load with `git show 9e986db8:<file>` into a scratch tree; reproduce the current
content with the bare command.

**`docs/references.md`'s residual is 82 of the run's 119, and is NOT swept.** The file is recorded
`Examined: yes` against a denominator of **10 counted assertions plus the repository-facing claims
enumerated by hand** — every one of which holds. The 82 unclassified candidates are bibliographic
tokens (author names, publication years, standards numbers) that the epic rules out of scope as the
wrong instrument for a bibliography. Stated here because the coverage row alone does not carry it.
