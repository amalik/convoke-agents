...
- **Tooling inventory — one new script, two extensions, and one thing deliberately not built.** Counted explicitly because the epic's requirement count grew twice under scrutiny and the mechanisation budget is the first place that shows up as unshipped work:
  ...
  | Coverage denominator (FR8/FR10) | ~~**Extension** to `docs-audit.js`~~ **AMENDED 2026-09-13 (Story 1.7): a SEPARATE gate, `scripts/audit/coverage-denominator.js`.** Extending `docs-audit.js` is not possible without breaking it: its `USER_FACING_DOCS` is a different set (9 of its entries are absent from the coverage table and 7 table rows are absent from it, so neither contains the other), and `tests/unit/docs-audit.test.js` pins that array's length against prose in `BMAD-METHOD-COMPATIBILITY.md`, so widening it turns that test red and falsifies a shipped sentence. The derivation described below is unchanged — only the host is; the in-scope column is derived from the filesystem minus a declared exclusion list, so a new document appears in the table unexamined on its own |
