...
| T4 | Migration idempotency CLI test — new `tests/integration/migrate-artifacts-idempotency.test.js` exercises `convoke-migrate --apply --force` twice against a real git-initialized fixture (ungoverned `prd-gyre.md` → `gyre-prd.md` rename). 4 tests: first-run rename + commits, second-run no-op + exit 0, no-new-commits proof, third-run resumes on added ungoverned file. Functional idempotency verified. Surfaced a latent UX bug (migration's own ADR + rename-map flagged as ungoverned on re-run) — logged to `deferred-work.md`. | 2026-04-19 | 2.4 | convoke |
...
| 2026-04-19 | **A24 shipped (Vortex audit expansion / IN-12).** v2 Covenant audit refresh executed. 3 new Vortex workflows (assumption-mapping, empathy-map, hypothesis-engineering) scored at step-01 = 21 cells. **T1 FIRES for Vortex × Right to pacing** (25% compliance, 1/4 PASS at N=4). Vortex team compliance: 25/28 cells (89%); 6 of 7 rights at 100%. 2 new retrofit cells added to Epic 2 Story 2.1 scope: assumption-mapping step-01 + hypothesis-engineering step-01 (shared HC-schema-at-step-01 anti-pattern — both dump 6-9 novel concepts in §3 Input Validation). Empathy-map is the sole Vortex pacing PASS. Report: `convoke-report-operator-covenant-audit-vortex-2026-04-19.md`. Single-reviewer limitation per A10 declared — T1-FIRES verdict is provisional pending 3-cell reproducibility pass before Epic 2 Story 2.1 retrofit commits. 3 rubric ambiguities logged as backlog-intake candidates (compound-concept counting, HC-schema enumeration treatment, oc-1-1 line 367 vs 271 consistency). A24 moved Fast Lane → §2.5 Completed. |
