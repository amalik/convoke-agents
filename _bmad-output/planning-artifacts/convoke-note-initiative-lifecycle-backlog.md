...
|----|-------------|---|---|---|---|-------|-----------|--------|--------------|
| A35 | Operator-visible boundary formalization — extend Checklist §171 "Operator-facing vs agent-facing text" with exhaustive enumeration (frontmatter, HTML comments, Steps Overview, fenced code blocks, `<critical-rules>`, `SYSTEM:`, INITIALIZATION blocks, `Note:` blocks) or a semantic test ("agent-facing = imperative addressee is the LLM and not rendered/echoed to the operator"). §171 is example-based; A33's Scope paragraph delegates to it and now load-bears on the determination. | 4 | 1 | 80% | 1 | 3.2 | convoke | Backlog | deferred-from: A33 Round 2 · bundles-with: A36 |
...
| A36 | Sibling files (`template.md`, `validate.md`, `*.template.md`) silently escape Checklist §2.4 Layer 1/2 enumeration — §2.4 Layer 1 names only `workflow.md`; Layer 2 is `steps/**.md`. Workflow-root companions are neither, so operator-visible concept-introductions evade OC-R7 (e.g., empathy-map's `validate.md:57-66` names "Says/Thinks/Does/Feels/Pains/Gains" dimensions). Fix: extend Layer 1 to cover workflow-root companions OR explicit §2.4 exclusion rule. | 3 | 1 | 80% | 1 | 2.4 | convoke | Backlog | deferred-from: A33 Round 2 · bundles-with: A35 |
