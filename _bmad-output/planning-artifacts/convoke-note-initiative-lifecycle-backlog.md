...
|----|-------------|---|---|---|---|-------|-----------|--------|--------------|
| A29 | Covenant audit third-workflow structural-diversity criterion — require ≥1 structurally-different skill per audit to avoid pattern-confirmation bias. Add to Compliance Checklist sample-selection rules. [rescored 2026-04-19: 2.4→4.8] | 3 | 1 | 80% | 0.5 | 4.8 | convoke | Backlog | deferred-from: A24 Round 1 |
...
| I67 | Cross-platform `npm test` script glob expansion — scripts in `package.json` use `tests/unit/*.test.js` that only expands under POSIX shells; Windows `cmd.exe` passes literal to `node --test` → gate fails. Fix with Node-side glob (e.g., `glob` package) or cross-platform shell wrapper. Touches ~5 scripts. [rescored 2026-04-19: 3.6→4.2] | 3 | 2 | 70% | 1 | 4.2 | convoke | Backlog | deferred-from: Story B.4 defer |
| A28 | Covenant audit step-01 sampling-bias rule — future Vortex audits must cover ≥2 steps per workflow OR explicitly scope to step-01 with documented rationale. Amend Compliance Checklist's §Representative-skill selection. [rescored 2026-04-19: 1.6→3.2] | 4 | 1 | 80% | 1 | 3.2 | convoke | Backlog | deferred-from: A24 Round 1 |
