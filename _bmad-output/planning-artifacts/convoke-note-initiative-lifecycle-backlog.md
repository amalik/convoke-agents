...
|----|-------------|---|---|---|---|-------|-----------|--------|--------------|------------------|
| BUG-2 | ADR generation silently fails on re-run with byte-identical content — `scripts/migrate-artifacts.js:402-404` downgrades `git commit`-on-empty-tree to warning + exit 0. Surfaced by T4's warning-guard firing. 3 fix paths: `git commit --allow-empty`, detect-and-skip no-op before git, or monotonic counter in ADR content. | 3 | 1 | 90% | 1 | 2.7 | convoke | Backlog | deferred-from: T4 Round 1 |

**Note:** Bug Lane has one active entry (BUG-2, logged 2026-04-19 from T4 Round 1 review). BUG-1 shipped 2026-04-18 — see §2.5. Latent risks (YAML parsing, atomic writes, etc.) sit in Fast Lane as preventive hardening.
...
|----|-------------|---|---|---|---|-------|-----------|--------|--------------|
| A27 | Staleness pre-flight — Phase 1 — new `project-context.md` rule `staleness-preflight-for-backlog-pickup` scoping 3 manual checks (existence/dependencies/code-anchor validity) for items qualified > 3-5 days ago + one-line reminder in `bmad-help`. Contributor-only reach — the rule is a convention-test whose evidence shapes Phase 2's skill-mode API. | 3 | 1 | 90% | 0.5 | 5.4 | convoke | Backlog | — |
| I64 | `runScript` helper returns `exitCode: null` on `execFile` timeout — surface `timedOut`/`signal` as distinct return fields so tests get meaningful timeout diagnostics instead of misleading `null !== 0` failures | 5 | 0.5 | 90% | 0.5 | 4.5 | convoke | Backlog | deferred-from: T4 Round 1 |
...
| A29 | Covenant audit third-workflow structural-diversity criterion — require ≥1 structurally-different skill per audit to avoid pattern-confirmation bias. Add to Compliance Checklist sample-selection rules. | 3 | 0.5 | 80% | 0.5 | 2.4 | convoke | Backlog | deferred-from: A24 Round 1 |
...
| A28 | Covenant audit step-01 sampling-bias rule — future Vortex audits must cover ≥2 steps per workflow OR explicitly scope to step-01 with documented rationale. Amend Compliance Checklist's §Representative-skill selection. | 4 | 0.5 | 80% | 1 | 1.6 | convoke | Backlog | deferred-from: A24 Round 1 |
...
| A25 | Decision-support archetype addition for v2 Covenant audit — add creative-problem-solver or innovation-strategy to break enhance-backlog single-sample-with-COI dependency | 4 | 0.5 | 80% | 1 | 1.6 | convoke | Backlog | deferred-from: oc-1-1 Round 3; renamed from A9 on 2026-04-19 (collision with v6.3 Adoption A9/WS4) |
...
| 2026-04-19 | **A24 shipped provisional (Vortex audit expansion / IN-12).** v2 Covenant audit refresh executed. 3 new Vortex workflows (assumption-mapping, empathy-map, hypothesis-engineering) scored at step-01 = 21 cells. **T1 FIRES for Vortex × Right to pacing** (provisional — 25% compliance, 1/4 PASS at N=4 Vortex-audited skills). Vortex team compliance: 25/28 cells (89%). Per-right fail counts (across N=4 Vortex skills): R1 default 0/4, R2 full-universe 0/4, R3 rationale 0/4, R4 completeness 0/4, R5 pause 0/4, R6 next-action 0/4 (all 100% compliance); **R7 pacing 3/4 (25% compliance)** — sole concentrated bottleneck. Under strict §2.6 counting, assumption-mapping + hypothesis-engineering step-01 introduce > 10 and > 15 novel concepts respectively (vs charitable grouping: 6 and 7). Empathy-map PASS is reading-dependent per §9 ambiguity #5 (PASS under broader §2.6 reading; FAIL under narrower — T1 fires either way). 2 new retrofit cells flagged for Epic 2 Story 2.1 scope (pending ratification): assumption-mapping step-01 + hypothesis-engineering step-01 (shared HC-schema-at-step-01 anti-pattern). Report: `convoke-report-operator-covenant-audit-vortex-2026-04-19.md`. **Proposed mitigation gates** (pending Epic 2 ratification, report §8.1): G1-G4 framing + gate-refinement candidates logged as IN-56 (scope: G2 "independent" definition, G1 cell composition, subsumption conditionality, empathy-map drift G5, baseline ratification scope). Code review Round 1 applied 14 patches + 3 decision-needed resolutions + 4 defers. Round 2 applied 16 additional patches + 2 decision resolutions (strict §2.6 counting; defer gate refinements) + 4 new ambiguity-based backlog intakes IN-53/54/55/56. A24 moved Fast Lane → §2.5 Completed (provisional — denotes shipped artifact with deferred-resolution items pending). |
