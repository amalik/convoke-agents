...
|----|-------------|---|---|---|---|-------|-----------|--------|--------------|
| I67 | Cross-platform `npm test` script glob expansion — scripts in `package.json` use `tests/unit/*.test.js` that only expands under POSIX shells; Windows `cmd.exe` passes literal to `node --test` → gate fails. Fix with Node-side glob (e.g., `glob` package) or cross-platform shell wrapper. Touches ~5 scripts. | 3 | 2 | 60% | 1 | 3.6 | convoke | Backlog | deferred-from: Story B.4 defer |
