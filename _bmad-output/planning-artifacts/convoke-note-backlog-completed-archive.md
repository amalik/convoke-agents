...
### Addendum — 2026-09-12, after Round 1 review

**Two claims in the entry above were false. Per the append-only rule they stand as written; this addendum corrects them.**

1. **"Six tests, all six red against the previous derivation" — false, and it is the second time this session.** The six tests do fail against the previous file, but they fail on the **missing export** — `validCountsFor` was not exported — not because the old derivation behaved differently. A minority of them would pass against the old three-named-array logic if it were merely hoisted into the same signature; *R2 established that the exact number depends on whether the hoist is written defensively, which the original claim did not say, so it is left unquantified here rather than restated with a second number.* A missing export is not a behavioural difference, and reporting it as one overstates what the suite pins. The identical overstatement was made about `T146`'s tests earlier the same day and corrected there too; applying the lesson in one row did not apply it in the next.
   **What the suite does pin, behaviourally:** a fourth team's roster makes its own size and both new totals valid without editing the checker, and — added at R1 — the `endsWith`-vs-`includes` and `Array.isArray` mutants, both of which **survived** the original suite. One of the original tests could not reach the guard it claimed to cover: it seeded `AGENT_IDS`, which fails the suffix filter before `Array.isArray` is ever consulted, so it asserted something trivially true under a title claiming coverage the suite did not have.
2. **The workflow half was retracted, and the retraction was wrong — re-filed as `T150`.** The retraction correctly disproved the *evidence* (37 was a count of workflow directories, not of registry rosters; `_portability`, `_artifacts` and `_enhance` have no agents and appear in the registry zero times). It then treated that as disproving the *finding*. It does not. Loom's `team-factory` — the agent in `EXTRA_BME_AGENTS` — **does** own a workflow, `_bmad/bme/_team-factory/workflows/add-team/`, and no `EXTRA_BME_WORKFLOWS` export exists. The true agent-owned total is 22 + 7 + 1 = **30**, and 30 is **rejected** — structurally identical to 12 being rejected, which is exactly what `T146` was. This entry closed asserting `validWorkflowCounts` was fine while the same defect it was closing sat live on the other axis.
   **The generalisation cannot fix it.** The derivation enumerates registry exports; the export does not exist. The fix belongs in `agent-registry.js`, and adding the export changes what `refresh-installation.js` walks — which is why `T150` is filed rather than patched here.

**Two further rows were filed from operator rulings taken at this review, not from defects in the change.** `T151` — the ruling was to **keep** the `EXTRA_` prefix as the registry's marker for a non-team roster and document it as a convention the generator must respect; `derivePrefix()` has no reserved word list, so a team named `extra-helm` emits `EXTRA_HELM_AGENTS` and silently leaves the team total (measured: valid `{1,3,4,7,14,15}` becomes `{1,3,4,7,11,15}` — the true total 14 disappears and the stale 11 returns). `T152` — the ruling was to **file, not solve**, the version-scope gap: `UPDATE-GUIDE.md:109` and `:123` state agent counts under `### From v1.7.x to v3.0.0` and `### From v1.0.x to v3.0.0` headings, are true of what those migrations produced, and go red the day a fourth team registers. The test that pins that behaviour was reworded at R1 — it had asserted the outcome was *desirable* ("the OLD team total must go stale once a team is added"), which is a position this repo has not taken and which `T152` exists to decide.

**Still true and unchanged:** the generalisation ships correctly, is order-independent by construction, is registry-injectable so it can be tested against rosters the real registry does not have, and the real valid sets are unchanged except for `1`. `lint`, `docs:audit`, `backlog-integrity` and the unit suite are all green; run them rather than trusting a count quoted here — the first draft of this line quoted the *pass* count as the suite size.

### Addendum — 2026-09-12, after Round 2 review

**Round 2 ran three blind layers. All three found the code clean** — the adversarial layer killed seven mutants against `validCountsFor` and reported no logic defect; the auditor confirmed the append-only rule was honoured with zero deletions, all three new rows arithmetically exact, and each mutant uniquely killed by the test its own comment names. **Every one of the ten findings was in prose, and seven were the same class:** a derived figure, command or output transcribed into an artifact whose reader re-derives it. Specifically: a *"Reproduce"* command written without its `suffix` argument, which returns the empty set and so reads as **disproving** the row it was filed to prove; a transcribed command output given in the wrong order; the scope word *"any size"* in `T152`, falsified by a fourth team whose roster happens to equal the cited figure (it stays valid — as that roster's own size, for an unrelated reason, so the check passes by coincidence); a quoted `TypeError` message matching only half the tests it described; and a Change Log entry misquoting the very sentence it was correcting.

**This is the third time the lesson has been written down and the first time it was applied by deletion.** The two prior recurrences were patched instance-by-instance. The three rows now name the call to make and transcribe no figures, outputs or commands at all.

**Two substantive corrections, both to claims that were load-bearing:**

1. **`T150` was rediscovered, not discovered.** The archive's committed `T146` entry already stated that the workflow gap was live and named the true roster — and it was committed **in `e3106432` itself**, the commit whose `T148` entry retracted that very finding. That commit contains both the claim and its retraction, which is sharper evidence for the row than anything R1 produced, and the first filing omitted it.
2. **`T150`'s stated reason for filing rather than patching was false.** It claimed adding an `EXTRA_BME_WORKFLOWS` export *"changes installer behaviour"*. `refresh-installation.js:12` destructures eight **named** exports and performs no dynamic enumeration; the exact analogue `GYRE_WORKFLOWS` is consumed by nothing outside `agent-registry.js`. A new export would be inert until wired. The honest reason is a design decision about where `add-team` belongs.

**One defect was in the source, not the record, and is the most consequential of the round.** `scripts/docs-audit.js` carried a comment above `validWorkflowCounts` reading *"That half of the row was retracted. What survives is growth"* — telling the next maintainer the workflow axis was settled, positioned on the exact line `T150` says is wrong today, and edited during R1 remediation without its conclusion being revisited. It now states the axis is known wrong and cites the row. Two further comments asserted rulings without naming the rows that made them (`T151`, `T152`); both now cite by ID, matching how the same file already cites `T146` and `T148`. A comment claiming `EXTRA_` had *"exactly one instance"* was also wrong as an unqualified count and has been qualified.
...
**Defect.** `{spec_path}`, `{context_path}` and `{scope_path}` were documented as repo-relative, so
they resolved against the executor's working directory: a `run:` block executed from anywhere but the
repository root throws ENOENT, and `initContext` run from two directories creates two different
context files. `{spec_path}` was defined only in `step-04-generate.md` while `step-02-connect.md` and
`step-05-validate.md` passed it to commands — neither had a placeholder table at all.

**Fix.** The step files document the `{project-root}`-prefixed form; `run-context.js` refuses a
relative path, and a path still holding an unsubstituted `{…}`, in `loadSpec`, `initContext`,
`readContext`, `recordContext` and `writeAtomic`. The producer (`step-01` §5), the two entry points
where an operator supplies a spec path (`step-00-route.md`, `templates/team-spec-template.yaml`) and
step-04 §3a's instruction to BMB were corrected to match.

| Fact | Command |
|---|---|
| The six placeholders on the test's literal list are defined where they are named, and resolve under `{project-root}/` | `node --test tests/team-factory/add-team-placeholders.test.js` |
| A relative or unsubstituted path is refused at every entry point | `node --test tests/team-factory/run-context.test.js` |
| The suite is green | `npm test` **and** `npm run test:p0` |

**Mutants → the test that kills each** (run on a full copy, control first):

| Mutant | Killed by |
|---|---|
| any definition losing its `{project-root}/` prefix, or gaining one only as trailing prose | that file's absoluteness test |
| any definition row deleted while `run:` blocks still pass it | that file's defines-what-it-names test |
| a step file deleted, renamed, or added | the directory-contents test |
| a new `{…_path}` / `{…_directory}` named anywhere | the path-shaped-name test |
| a definition hidden in a code fence or an HTML comment | both (stripped before parsing) |
| any `assertAbsolute` call site removed | `run-context.test.js` |
| `initContext`'s guard removed (it mkdirs before it writes) | the parent-directory assertion |

**Three rounds of the same blindness, worth stating once.** The guard's placeholder list was first
derived from the step files' own definitions, so deleting a definition removed it from the set. Fixed
by making it literal. Then the FILE list was read from the directory, so deleting a whole step file
was green. Both lists are literal now and the directory is checked against the file list. A set built
from the thing under test cannot see a deletion, at any level.

**Retracted.** An earlier version of this entry, and comments in two shipped source files, said the
relative path "is how `checkConfig`/`checkActivation`/`checkRegistryWiring` reported false failures on
a correctly generated team". Not derived and not true: `run-context.js` was created 2026-09-14, the
day T168 was filed from that story's own Round 3, so the relative form never ran in an operator's
hands. `step-05-validate.md:14` records the actual cause — there was no context file at all. Also
retracted: `{module_root}` and `{registry_path}` were documented *absolute in prose*, not "still
relative"; what they lacked was the substitutable `{project-root}/` form the guard can check.

**Residue, filed rather than patched:** T212 (values recorded INTO the context file are unenforced),
T213 (`{team}`, `{path}`, `{output_directory}` are path-valued and defined nowhere), T214 (the
