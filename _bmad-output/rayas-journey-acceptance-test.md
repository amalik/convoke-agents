...
|------|----------|--------|
| User Trust | Zero stale references, zero broken links, accurate first impression | PARTIAL — Automated gate passes (docs-audit zero findings, zero broken links). However, manual walkthrough found stale content outside automated scope: EMMA-USER-GUIDE.md has wrong menu items (O1), FAQ references wrong journey path (O9). PRD gate requires "zero stale references" which is binary — these stale references exist. README lines 182-188 actively link to stale user guides, routing users to incorrect information. First impression for Claude Code users is accurate; Claude.ai users lack activation guidance (O3). |
...
| Engineering Confidence | Content correctness in CI, CLI coverage 85%+, zero bugs tests should catch | PASS — docs audit zero findings, 293/293 unit tests pass, CI gate is functional. The stale user guide (O1) is not covered by automated docs-audit because user guides are not in the USER_FACING_DOCS list |
...
|---|------|------------|-----------------|
| O1 | EMMA-USER-GUIDE.md stale menu | Users following the README `cat` command see the correct agent menu. However, README lines 182-188 ("Agent Guides" section) actively link to user guides, making them a discoverable resource — not just a secondary one. An outsider who clicks these links encounters stale menu items that contradict the actual agent. The primary activation path (`cat` command) works correctly, but the guide path is a realistic user route. | **Backlog:** Update all 7 user guides to match v1.6.4 agent menus. Add user guides to docs-audit scope. Consider removing or demoting guide links from README until guides are current. |
