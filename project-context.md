...
- **Batch the remediation; do not apply findings one at a time as they arrive.** Collect every layer's findings for the round, design ONE coherent remediation, then apply it. Incremental fixing inside a round is how fixes collide: in `dist-2-2` Round 1 added an unterminated-fence tripwire and a zero-extraction guard in the same round, each individually correct, and the guard **swallowed the tripwire's finding** and exited 2 — which the harness treats as advisory, so the run reported nothing at all. Neither diff read wrongly on its own. Round 2 then fixed one of the two sibling guards and left the other, and Round 3 found the same defect alive in it. A remediation designed as a whole would have seen both.
- **When a fix keeps leaking in the same place, suspect OVER-BUILD, and prefer deletion to a further rewrite.** Repeated defects concentrated in one component usually mean it does more than its acceptance criterion asked. `dist-2-2`'s self-referential-URL half leaked in two successive rounds — `http`/`https` and `www.` found in Round 2, then host case and port still open in Round 3, each a case where two strings differ and the URLs are the same — because it had been built well past AC5: npm bare shorthands, transport ports and multi-segment repository paths, none specified, none used by the package, and each a fresh way to return a **confident wrong answer that passes a fail-closed guard**. `repositoryIdentity('../owner-repo')` returned a real identity. Round 3's instrument change was to **delete** the excess, not rewrite it a third time. Narrowing an implementation back to what the AC specifies is *restoring* scope, not cutting it, and needs no ruling — but it does need a **record**: name what was removed and why in the story, so the narrowing is visible rather than silent. If the excess turns out to be load-bearing for a consumer the AC does not mention, that is a ruling, not a deletion.
...
- **At story close, audit the consumers — not only the diff.** Every round above reviews a diff, and a diff cannot show a caller that was never edited. When a story deletes, renames or changes the behaviour of an exported function, a context key, a `run:`/CLI contract or a documented capability, run one extra layer before it moves to `done`: enumerate those changes, then find **every consumer across the whole repository** — code, tests, workflows, agent menus, `module-help.csv`, docs — including consumers no hunk touched, and judge each still correct. The layer must be independent of the implementer, and it must search for **behavioural claims in prose**, not only symbol names. This is an extra layer, not an extra round: it does not reset or extend the Round 1–3 bound.
...
**Why the consumer-audit clause.** In `tfr-1-1`, a whole-story consumer audit run at the operator's request after the story reached `review` found breaks that every diff-scoped round on that story had missed, because the broken call sites were never in a changed hunk: `[VT] Validate Team` could no longer pass, and step-02 §5's `run:` block could not run in the documented order (story §"Review Findings — operator-requested additional layers"). A third, the extension validators calling a check without the baseline it now required, was found only by reading the whole validator file during that story's Task 8 (§Completion Notes, "Found during the run"). In `tfr-2-1` the story's own consumer audit searched symbol names and missed `docs/development.md`, which restated the deleted behaviour in prose containing none of them; the independent Acceptance Auditor's audit caught it (story §"Review Findings — Round 1"). Hence both conditions: whole-repository, and independent of the implementer.
...
- **Scope the consumer audit from the change, then search outward.** List what the story changed (exported symbols, context keys, `run:`/CLI contracts, documented capabilities), then `git grep` the whole repository for each — and for the *behaviour* each implements, phrased as a reader would describe it, since prose restating a deleted behaviour contains no symbol. Record every consumer with a disposition (updated, deleted, historical record left as-is). **Falsification:** if two consecutive story-close consumer audits find nothing the diff-scoped rounds had not already found, relax this clause to operator-requested.
...
**Why.** Nine failures, every one of which produced a confident wrong answer. The first four fell in three days; the fifth is from `dist-2-5` (2026-09-02) and is the same error one level in. The last four are from `dist-epic-2`'s closing stories and T102, **after** this rule was already written and being followed — which is the point: the rule did not stop them, *running the falsification* did. Every one was caught by execution, none by reading:
...
| Bare-identifier extractor run over the pin (`dist-2-7`, 2026-09-05) | the pin matches; the interpolation site is covered | The extractor could not see **quoted** keys, so the site it was written for was invisible to it. It matched, and it was blind |
| Ratchet plant + `node --test` → `16 pass` (`dist-2-8`, 2026-09-06) | "the planted defect is caught" | The plant script **threw before planting anything**. Sixteen tests passed against an unmodified tree. The number was real and meant nothing |
| `declaredUnits({registry:{}})` asserting C1 fires (T102 (e), 2026-09-07) | "the fix is pinned" | The defect was in the CALLER, which substituted a literal `{}` instead of calling `declaredUnits` at all. The test exercised the callee and stayed green when the fix was reverted |
| Probe agent pushed into `agent-registry.js`, harness → `tree=0` (T102 NFR10, 2026-09-08 — in-session, no story record; the mechanism is checkable at `refresh-installation.js:854`) | "the gate ran and the tree is clean" | **The mutant was absorbed by a derived expectation.** Wrapper generation READS that registry (`refresh-installation.js:854`), so the probe entered the *expected* set and the *actual* set together. The two sides agreed because they share a source |
...
- **For a VALIDATOR, an assertion of "no finding" tests nothing.** A check that *skipped* an input produces exactly the observation a check that *validated and passed* it produces, so a test whose expected value is the empty result cannot distinguish the two. Assert with an input that MUST be reported — a missing target, a broken reference — so that a finding has to appear. In `dist-2-2` every self-referential test used a target that existed; folding owner/repo case-sensitively therefore survived the entire suite, because the resulting silent skip and the intended pass were the same green. Rewritten against a missing target, the same edit dies immediately.
- **Cite the TEST, never a mutant identifier.** Mutation harnesses are scratch scripts and are not committed, so an `M14`-style identifier is a pointer to nothing: a reader cannot reproduce the row, and per `verification-claims-must-name-their-evidence` a pointer to nothing reads as evidence and is worse than none. This recurred in three consecutive `dist-2-2` rounds — the last time inside the very paragraph written to remove the previous such citation. Key the matrix on **(edit to make → test that goes red)**; both halves exist in the repository, so any row can be re-run with one command.
- **A survived mutant may be an EQUIVALENT mutant, and the difference must be established rather than assumed.** Where guards are layered, no single line is solely load-bearing and removing any one of them changes no behaviour — that is redundancy, not a coverage hole. Distinguish the two with a **combined** mutant that strips the whole defence at once: in `dist-2-2` three separate edits each survived, while removing all of them together produced `https://github.com/../owner-repo/` — the exact defect under review — and was killed by two named tests. Record the survivor as redundant. Do **not** write one test per redundant line so that every row shows a kill; that manufactures the appearance of coverage instead of the fact of it. This refines the "sole executioner" bullet above: that bullet is about a test nothing needs, this one is about a *line* whose neighbours already enforce its property — delete-or-rewrite applies to the first, honest annotation to the second.
...
- **The mutant must not be ABSORBED by a derived expectation.** Where a check compares two sets, establish that your injection lands on ONE side before reading the result. T102's NFR10 demonstration pushed a probe agent into `agent-registry.js` expecting a missing wrapper — but the installer *generates* wrappers from that same registry, so the probe joined the expected and the actual set at once and the gate correctly reported clean. Read as "gate ran, tree=0, fine", it would have shipped a blocking gate whose demonstration proved nothing. **Ask what the expectation is derived FROM.** If the answer includes the thing you just mutated, the mutant is inert; inject at the point where the two sides genuinely disagree — there, an installer that DROPS a unit the registry declares.
- **A demonstration that trips several gates at once has not isolated yours.** T102's replacement injection produced `tree=1` and a red run — and also `doctor=1`, so it proved the harness fails on a broken tree, not that the `$TREE` wiring is what fails it. Isolating it needed a defect the sibling gate treats as a soft warning. When a verdict is a conjunction, a demonstration that moves two terms tests neither.
