...
| `{config_path}` | the config reference **as agents write it**: `{project-root}/_bmad/bme/_{team_name_kebab}/config.yaml`. Pass this placeholder form, not a resolved path — `activation-validator.js` check 2 **requires** it: every occurrence of the config path in an activation block must carry the `{project-root}/` prefix (T214), and an unprefixed one fails even if a prefixed one appears elsewhere in the block |
