...
**Mapping rules:**
- The Covenant (`covenant-operator.md`, Story 1.4 output) MUST pin each right to a slug via frontmatter or section metadata. Slugs are the reference-invariant.
...
*(Added 2026-04-25 via A41+A42 bundle ship. Resolves 5 HIGH + 4 MEDIUM findings from A40 R2 + 7 from A39 R2/R3. Forward-only application: v4+ audits use these clarifications; pre-v4 audits stay locked under G4 mitigation gate per their original methodology version. See [oc-publication-gate-rigor-a41-a42.md](https://github.com/amalik/convoke-agents/blob/main/_bmad-output/implementation-artifacts/oc-publication-gate-rigor-a41-a42.md) for full A41+A42 spec + 13 Pre-Author Decisions.)*
...
|------|-------------|--------|--------|
| 2026-09-07 | rule-content-edit | **Document moved to `_bmad/bme/covenant/compliance-checklist.md` so it ships to operators (story dist-2.3b).** Three reference updates, no rule semantics changed: the Covenant is now cited as `covenant-operator.md` in the slug-pinning rule, and the two outbound links to `_bmad-output/implementation-artifacts/` became self-referential absolute URLs because that directory does not ship — left relative, they would have been unfollowable for every operator. *The allowed change types have no slot for a reference-path update; `rule-content-edit` is the nearest, and this note records that the rule text changed only in how it names a file.* | dist-2.3b |
